...
function sectionForKey(key) {
  ...
  if (key.startsWith("STANDUP_") || key.startsWith("ESSENTIAL_STANDUP_"))
    return "STANDUP";
  if (key.startsWith("A2A_") || key.startsWith("REFLECTION_")) return "A2A";
  if (
...
out.push(
  "# VINCE — Environment Variables (reordered; same as .env.example structure)",
);
out.push(
  "# =============================================================================\n",
);
...
console.log(
  "Reordered .env into sections. Backup not created; check git diff.",
);
process.exit(0);
