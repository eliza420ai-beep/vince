...
export function validateWttPick(input: unknown): WttValidationResult {
  ...
  if (
    ...
    (!isFiniteNumber(raw.schemaVersion) || raw.schemaVersion < 1)
  ) {
    pushError(
      errors,
      "schemaVersion",
      "schemaVersion must be a finite positive number when present",
    );
  }
  ...
  if (
    raw.catalystSources != null &&
    (!Array.isArray(raw.catalystSources) ||
      raw.catalystSources.some(
        (v) =>
          typeof v !== "string" ||
          !["headlines", "ct", "polymarket", "generic"].includes(v),
      ))
  ) {
    pushError(
      errors,
      "catalystSources",
      "catalystSources must be an array of headlines|ct|polymarket|generic",
    );
  }
  ...
  return {
    ...
    value: {
      ...
      catalystSources: raw.catalystSources as WttCatalystSource[] | undefined,
    },
  };
}
...
function coerceLegacyWttPick(input: unknown): Record<string, unknown> | null {
  ...
  if (raw.schemaVersion == null) {
    raw.schemaVersion = 1;
  } else if (!isFiniteNumber(raw.schemaVersion) || raw.schemaVersion < 1) {
    return null;
  }
  if (!validateEnum(raw.primaryDirection, ["long", "short"] as const)) {
  ...
  if (
    raw.catalystSources != null &&
    (!Array.isArray(raw.catalystSources) ||
      raw.catalystSources.some((v) => typeof v !== "string"))
  ) {
    delete raw.catalystSources;
  }
