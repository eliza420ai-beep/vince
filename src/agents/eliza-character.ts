...
/**
 * Eliza's character definition (personality, plugins, system prompt, knowledge).
 * Used only by the Eliza agent — see src/agents/eliza.ts which wires this + init + plugins.
 *
 * Eliza — 24/7 RESEARCH & KNOWLEDGE EXPANSION
 * Core use case: works the knowledge folder 24/7 and ingests content you send—
 * especially YouTube. Same DNA as VINCE: trade well, live well. She suggests and
...
export const elizaCharacter: Character = {
  ...
  plugins: [
    ...
    // Discord: prefer ELIZA_* for this agent. DISCORD_* is fallback for single-bot setups only.
    // For VINCE + Eliza in the same server use two apps: set ELIZA_DISCORD_* and VINCE_DISCORD_* (different IDs); avoid using generic DISCORD_* for Eliza when both run.
    ...((process.env.ELIZA_DISCORD_API_TOKEN?.trim() || process.env.DISCORD_API_TOKEN?.trim()) ? ['@elizaos/plugin-discord'] : []),
  ...
  settings: {
    secrets: {
      // Eliza's Discord: ELIZA_* takes precedence so each agent can have its own bot when running both in one server.
      ...(process.env.ELIZA_DISCORD_APPLICATION_ID?.trim() && { DISCORD_APPLICATION_ID: process.env.ELIZA_DISCORD_APPLICATION_ID }),
      ...(process.env.ELIZA_DISCORD_API_TOKEN?.trim() && { DISCORD_API_TOKEN: process.env.ELIZA_DISCORD_API_TOKEN }),
      ...(process.env.DISCORD_APPLICATION_ID?.trim() && !process.env.ELIZA_DISCORD_APPLICATION_ID?.trim() && { DISCORD_APPLICATION_ID: process.env.DISCORD_APPLICATION_ID }),
      ...(process.env.DISCORD_API_TOKEN?.trim() && !process.env.ELIZA_DISCORD_API_TOKEN?.trim() && { DISCORD_API_TOKEN: process.env.DISCORD_API_TOKEN }),
    },
