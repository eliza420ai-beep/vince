...
const COLLECTION_CONTEXT: Record<
  string,
  { story: string; whyItMatters: string }
> = {
  cryptopunks: {
    story:
      "The original NFT PFP. 10,000 punks from 2017, predates ERC-721. Zombie, Ape, Alien = grails.",
    whyItMatters:
      "Cultural icon. Usually thick floor (100+ listed). Sub-40 ETH is the target entry.",
  },
  "official-v1-punks": {
    story:
      "The 'wrapped' V1 Punks from the original buggy contract. OG collector territory.",
    whyItMatters:
      "Much smaller supply than V2. Historical artifact for serious collectors.",
  },
  autoglyphs: {
    story:
      "First on-chain generative art. Algorithm IS the contract. Centre Pompidou, Whitney own pieces.",
    whyItMatters:
      "Only 512 exist. VERY thin floor (<10 listed). Grail-tier, move fast when listed.",
  },
  meebits: {
    story:
      "3D voxel characters by Larva Labs. Airdropped to Punk holders. Now under Yuga.",
    whyItMatters:
      "Bridge between 2D PFP and metaverse. More accessible than Punks.",
  },
  "beeple-everydays": {
    story:
      "Mike Winkelmann's daily art since 2007. 'First 5000 Days' sold for $69M at Christie's.",
    whyItMatters:
      "You're buying the streak, the discipline. Historical significance over single images.",
  },
  "chromie-squiggle-by-snowfro": {
    story:
      "Art Blocks genesis. Snowfro's signature. MoMA acquired 8. LACMA has them too.",
    whyItMatters:
      "~10K supply. Entry-level blue chip. The 'soul of Art Blocks'.",
  },
  "ringers-by-dmitri-cherniak": {
    story:
      "Strings wrapped around pegs. The Goose sold $6.2M at Sotheby's. LACMA acquired #962.",
    whyItMatters:
      "1,000 pieces. Art Blocks Curated flagship. Floor moves fast with thin liquidity.",
  },
  "fidenza-by-tyler-hobbs": {
    story:
      "Tyler Hobbs' masterpiece. Flow fields that feel alive. Consensus best of Art Blocks.",
    whyItMatters:
      "999 pieces. Limited supply = thin floor. True collector territory.",
  },
  "meridian-by-matt-deslauriers": {
    story:
      "Matt DesLauriers' flowing landscapes. Art Blocks Curated. Strong artist reputation.",
    whyItMatters:
...
export const vinceNftFloorAction: Action = {
  ...
  description:
    "Shows thin floor NFT buying opportunities with collection context",
  ...
  handler: async (
    ...
    try {
      ...
      if (opportunities.length === 0) {
        ...
        sections.push(
          "All tracked collections have thick floors. No immediate buying opportunities.",
        );
        sections.push("");
        sections.push(
          `_Tracking ${status.collectionCount} collections. Check back later._`,
        );
        sections.push("");
        sections.push("---");
        sections.push(
        ...
        for (const collection of opportunities) {
          const gapPct = (
            (collection.gaps.to2nd / collection.floorPrice) *
            100
          ).toFixed(0);
          const context = COLLECTION_CONTEXT[collection.slug];

          sections.push(
            `**${collection.name.toUpperCase()}** - ${collection.floorPrice.toFixed(2)} ETH`,
          );
        ...
        sections.push("---");
        sections.push(
          `Tracking ${status.collectionCount} collections. ${opportunities.length} have thin floors.`,
        );
  ...
  examples: [
    [
      {
        name: "{{user1}}",
        content: { text: "What are the NFT opportunities?" },
      },
      {
        name: "VINCE",
        content: {
          text: "**NFT FLOOR - Thin Floor Opportunities**...",
          actions: ["VINCE_NFT_FLOOR"],
        },
      },
    ],
