...
import type { Position, AggregatedTradeSignal, TradeSignalDetail } from "../types/paperTrading";
...
const SIGNAL_PATTERN_EXPLANATIONS: Record<string, (context: string, direction: string) => string> = {
  // Market Regime
  "ranging": (ctx, dir) => 
  ...
  "funding": (ctx, dir) => {
    ...
    const isNegative = ctx.includes("-") || ctx.toLowerCase().includes("negative");
    const isHigh = ctx.toLowerCase().includes("high") || ctx.toLowerCase().includes("extreme");
  ...
  "open interest": (ctx, dir) => {
    const rising = ctx.toLowerCase().includes("rising") || ctx.toLowerCase().includes("increas");
    if (rising) {
...
export function explainSignal(signal: string, direction: string): string {
  ...
  for (const [pattern, explainer] of Object.entries(SIGNAL_PATTERN_EXPLANATIONS)) {
...
export function buildTradeThesis(position: Position): string {
  ...
  // Detect key themes
  const hasRegime = signalsStr.includes("regime") || signalsStr.includes("ranging") || signalsStr.includes("trending");
  const hasFunding = signalsStr.includes("funding");
  const hasDeviation = signalsStr.includes("sma") || signalsStr.includes("deviation");
  const hasWhale = signalsStr.includes("whale");
  const hasLS = signalsStr.includes("l/s") || signalsStr.includes("long") || signalsStr.includes("short");
  
  let thesis = "";
  
  if (hasRegime && hasDeviation && direction === "short") {
    thesis = "Market is ranging (not trending), with price extended above its moving average. Classic mean reversion setup - betting on a pullback to the mean.";
  } else if (hasRegime && hasDeviation && direction === "long") {
    thesis = "Market is ranging with price dipped below its moving average. Buying the dip expecting a bounce back - mean reversion play.";
  } else if (hasFunding && hasLS && direction === "short") {
    thesis = "Funding data shows longs are crowded and paying premiums. L/S ratio confirms overleveraged bulls. Positioned to profit when they get flushed.";
  } else if (hasFunding && hasLS && direction === "long") {
    thesis = "Funding negative with shorts crowded. When shorts pay premiums to maintain positions, squeeze potential is high. Positioned for the squeeze.";
  } else if (hasWhale && direction === "long") {
    thesis = "Whale activity confirmed bullish positioning. Smart money leading the way - following their conviction with size.";
  } else if (hasWhale && direction === "short") {
    thesis = "Whale activity confirmed bearish positioning. Smart money getting defensive - following their lead to the downside.";
  } else {
...
export function formatPositionExplanation(position: Position): string {
  ...
  lines.push(`${dirIcon} **${position.direction.toUpperCase()} ${position.asset}** @ $${position.entryPrice.toLocaleString()}`);
  ...
  lines.push(`Size: $${position.sizeUsd.toLocaleString()} · ${position.leverage}x leverage`);
  lines.push("");
  
  // The thesis
  lines.push("**THE THESIS**");
  lines.push(`_Why ${position.direction.toUpperCase()} ${position.asset} makes sense:_`);
  lines.push("");
  ...
  const slPct = ((position.stopLossPrice - position.entryPrice) / position.entryPrice * 100);
  lines.push(`Stop-Loss: $${position.stopLossPrice.toLocaleString()} (${slPct >= 0 ? "+" : ""}${slPct.toFixed(1)}%)`);
  ...
  if (position.takeProfitPrices.length > 0) {
    ...
    position.takeProfitPrices.forEach((tp, i) => {
      ...
      lines.push(`  ${i + 1}. $${tp.toLocaleString()} (${tpPct >= 0 ? "+" : ""}${tpPct.toFixed(1)}%)`);
    });
  }
  lines.push("");
...
export function formatWhyNotTrading(signal: AggregatedTradeSignal): string {
  ...
  // Current bias
  const biasIcon = signal.direction === "long" ? "🟢" : signal.direction === "short" ? "🔴" : "⚪";
  const biasStrength = signal.strength < 50 ? "(weak)" : signal.strength < 70 ? "(moderate)" : "(strong)";
  lines.push(`Current Bias: ${biasIcon} **${signal.direction.toUpperCase()}** ${biasStrength}`);
  lines.push(`Strength: ${signal.strength.toFixed(0)}% (need 60%)`);
  lines.push(`Confidence: ${signal.confidence.toFixed(0)}% (need 60%)`);
  lines.push(`Confirming: ${signal.confirmingCount} | Conflicting: ${signal.conflictingCount}`);
  lines.push("");
  ...
  lines.push(`${dirOk ? "[X]" : "[ ]"} Direction: ${signal.direction.toUpperCase()}${!dirOk ? " (need clear direction)" : ""}`);
  lines.push(`${strengthOk ? "[X]" : "[ ]"} Strength: ${signal.strength.toFixed(0)}%${!strengthOk ? ` (${60 - signal.strength}% below threshold)` : ""}`);
  lines.push(`${confidenceOk ? "[X]" : "[ ]"} Confidence: ${signal.confidence.toFixed(0)}%${!confidenceOk ? ` (${60 - signal.confidence}% below threshold)` : ""}`);
  lines.push(`${confirmingOk ? "[X]" : "[ ]"} Confirming signals: ${signal.confirmingCount}${!confirmingOk ? ` (need ${2 - signal.confirmingCount} more)` : ""}`);
  lines.push("");
  ...
  if (!strengthOk) issues.push(`- Signal strength (${signal.strength.toFixed(0)}%) below 60% threshold`);
  if (!confidenceOk) issues.push(`- Confidence (${signal.confidence.toFixed(0)}%) below 60% threshold`);
  if (!confirmingOk) issues.push(`- Only ${signal.confirmingCount} confirming signal(s), need 2`);
  if (signal.conflictingCount > 0) issues.push(`- ${signal.conflictingCount} conflicting signal(s) reducing confidence`);
  ...
  if (!strengthOk) {
    ...
    if (signal.direction === "long") {
      triggers.push(`📈 **+${needed.toFixed(0)}% signal strength** could come from:`);
  ...
  if (!confirmingOk) {
    triggers.push(`✅ **Need ${2 - signal.confirmingCount} more confirming signal(s)**`);
