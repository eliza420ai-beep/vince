...
export async function runPreFlight(
    ...
    if (session.questions_for_next_session?.length > 0) {
      parts.push(
        "Questions for this session: " +
          session.questions_for_next_session.join("; "),
      );
      if (options.runWebSearchForQuestions) {
        for (const q of session.questions_for_next_session) {
          ...
          if (snippets.length > 0) {
            parts.push(
              `Answer for "${q.slice(0, 50)}...": ${snippets[0].slice(0, 200)}`,
            );
          }
        }
      }
  ...
  if (openRecs.length > 0 && runtime) {
    ...
    activeRecommendationsSummary =
      "Active recommendations: " + recParts.join("; ");
...
export async function getWalletActivitySummary(
    ...
    if (snippets.length > 0) {
      ...
      if (tokenMatch) {
        for (const t of tokenMatch) {
          ...
          if (key.length >= 2)
            tokensSeen.set(key, (tokensSeen.get(key) ?? 0) + 1);
        }
      }
    }
  }
  ...
  const convFromLog = logEntries
    .filter(
      (e) => e.source === "smart_wallet" && e.tags?.includes("convergence"),
    )
    .slice(-3)
...
export async function runCryptoIntelPostReport(
  ...
  if (xai) {
    try {
      const extractResult = await xai.generateText({
        ...
        system:
          "You output only valid JSON array. No markdown, no explanation.",
        model: GROK_MODEL,
        temperature: 0.1,
        maxTokens: 800,
      });
  ...
  const section8Match = fullReportMarkdown.match(
    /##\s*8\.\s*Today'?s?\s*Recommendations[\s\S]*?(?=##\s*9\.|##\s*10\.|$)/i,
  );
  ...
  if (section8Text && xai) {
    try {
      ...
      const extractResult = await xai.generateText({
        ...
        system:
          "You output only a valid JSON array. No markdown, no explanation.",
        model: GROK_MODEL,
        temperature: 0.1,
        maxTokens: 1500,
      });
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          const entries: RecommendationEntry[] = parsed
            ...
            .map((p) => ({
              ...
              action: String(p.action).toLowerCase() as
                | "buy"
                | "sell"
                | "watch",
              ...
              invalidation:
                p.invalidation != null ? String(p.invalidation) : undefined,
              status: "open" as const,
              ev_bull_pct:
                p.ev_bull_pct != null ? Number(p.ev_bull_pct) : undefined,
              ev_base_pct:
                p.ev_base_pct != null ? Number(p.ev_base_pct) : undefined,
              ev_bear_pct:
                p.ev_bear_pct != null ? Number(p.ev_bear_pct) : undefined,
              ev_bull_return:
                p.ev_bull_return != null ? Number(p.ev_bull_return) : undefined,
              ev_base_return:
                p.ev_base_return != null ? Number(p.ev_base_return) : undefined,
              ev_bear_return:
                p.ev_bear_return != null ? Number(p.ev_bear_return) : undefined,
            }));
          if (entries.length > 0) {
