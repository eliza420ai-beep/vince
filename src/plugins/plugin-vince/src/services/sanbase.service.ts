...
export class VinceSanbaseService extends Service {
  ...
  capabilityDescription =
    "Provides on-chain analytics from Santiment (network, dev, whale activity)";
  ...
  constructor(protected runtime: IAgentRuntime) {
    ...
    if (!this.apiKey) {
      logger.debug(
        "[VinceSanbaseService] SANTIMENT_API_KEY not set - API calls will fail",
      );
    } else {
    ...
    if (!isElizaAgent(runtime) && service.isConfigured()) {
      try {
        ...
        if (testData?.activeAddresses) {
          logger.info(
            `[VinceSanbaseService] ✅ API verified - BTC active addresses: ${testData.activeAddresses.toLocaleString()}`,
          );
        } else if (testData) {
          logger.info(
            `[VinceSanbaseService] ✅ API connected - network trend: ${testData.trend}`,
          );
        } else {
      ...
      logger.debug(
        "[VinceSanbaseService] No API key - Sanbase features disabled",
      );
    }
    return service;
  }
  ...
  private printDashboard(): void {
    ...
    if (!this.isConfigured()) {
      ...
      logLine("ALGO IMPACT:");
      logLine(
        "   • Used for: Exchange Flows (1.2x) → accumulation/distribution",
      );
      ...
      logEmpty();
      logLine(
        "⚠️ If you hit API limits: Sanbase adds no insight for that request.",
      );
  ...
  async printLiveDashboard(asset: string = "BTC"): Promise<void> {
    ...
    if (ctx.networkActivity) {
      const addrStr =
        ctx.networkActivity.activeAddresses?.toLocaleString() || "N/A";
      const trendEmoji =
        ctx.networkActivity.trend === "increasing"
          ? "📈"
          : ctx.networkActivity.trend === "decreasing"
            ? "📉"
            : "➡️";
    ...
    if (ctx.exchangeFlows) {
      const flowEmoji =
        ctx.exchangeFlows.sentiment === "accumulation"
          ? "🟢"
      ...
      logLine(
        `💱 Exchange: ${flowEmoji} ${ctx.exchangeFlows.sentiment.toUpperCase()} (net ${flowDir})`,
      );
    }
    if (ctx.whaleActivity) {
      const whaleEmoji =
        ctx.whaleActivity.sentiment === "bullish"
          ? "🐋"
          : ctx.whaleActivity.sentiment === "bearish"
            ? "🔴"
            : "⚪";
      logLine(
        `${whaleEmoji} Whales: ${ctx.whaleActivity.sentiment.toUpperCase()}`,
      );
    }
    sep();
    logEmpty();
    ...
    const tldrEmoji =
      tldr.includes("ACCUMULATION") || tldr.includes("bullish")
        ? "💡"
        : tldr.includes("DISTRIBUTION") || tldr.includes("bearish")
          ? "⚠️"
          : "📋";
    ...
    try {
      ...
      if (errorsOnly) {
        ...
        if (now - this.lastApiErrorLogTime >= ERROR_LOG_COOLDOWN_MS) {
          ...
          const isDateError = data.errors.every(
            (e: { message?: string }) =>
      ...
      if (hasErrors) {
        ...
        if (!isDateError) {
          ...
          if (now - this.lastApiErrorLogTime >= ERROR_LOG_COOLDOWN_MS) {
            this.lastApiErrorLogTime = now;
            logger.warn(
              `[VinceSanbaseService] GraphQL errors: ${JSON.stringify(data.errors)}`,
            );
          }
        }
      }
  ...
  async getNetworkActivity(asset: string): Promise<NetworkActivity | null> {
    ...
    const transactionVolume =
      result.data.transactionVolume?.timeseriesData || null;
  ...
  async getExchangeFlows(asset: string): Promise<ExchangeFlows | null> {
    ...
    const inflowVal = this.getLatestValue(
      result.data.inflow?.timeseriesData || null,
    );
    const outflowVal = this.getLatestValue(
      result.data.outflow?.timeseriesData || null,
    );
  ...
  async getDevActivity(asset: string): Promise<DevActivity | null> {
    ...
    const activityVal = this.getLatestValue(
      result.data.devActivity?.timeseriesData || null,
    );
    const contributorsVal = this.getLatestValue(
      result.data.contributors?.timeseriesData || null,
    );
  ...
  async getWhaleActivity(asset: string): Promise<WhaleActivity | null> {
    ...
    const countVal = this.getLatestValue(
      result.data.count?.timeseriesData || null,
    );
  ...
  async getOnChainContext(asset: string): Promise<OnChainContext> {
    const [networkActivity, exchangeFlows, devActivity, whaleActivity] =
      await Promise.all([
