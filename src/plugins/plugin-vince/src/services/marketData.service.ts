...
export class VinceMarketDataService extends Service {
  ...
  private dailyOpenPrices: Map<string, { price: number; date: string }> = new Map();
  ...
  static async start(runtime: IAgentRuntime): Promise<VinceMarketDataService> {
    ...
    logger.debug("[VinceMarketData] Service initialized with Open Window Trend Spotting");
    return service;
  }
  ...
  private updateDailyOpen(asset: string, currentPrice: number): number | null {
    ...
    if (!cached || cached.date !== today) {
      ...
      logger.debug(`[VinceMarketData] ${asset} daily open set to $${currentPrice.toFixed(2)}`);
      return currentPrice;
    }
   ...
   */
  async getEnrichedContext(asset: string): Promise<EnrichedMarketContext | null> {
  ...
  async getATRPercent(asset: string): Promise<number> {
    ...
    if (deribitService && (asset === "BTC" || asset === "ETH")) {
      try {
        const dvol = await deribitService.getVolatilityIndex(asset as "BTC" | "ETH");
        if (dvol?.current) {
   ...
   */
  async estimateRSI(asset: string): Promise<{ rsi: number; signal: "overbought" | "oversold" | "neutral" }> {
    const ctx = await this.getEnrichedContext(asset);
   ...
   */
  async getRSIAdjustment(asset: string, direction: "long" | "short"): Promise<{ multiplier: number; reason?: string }> {
