...
class DynamicConfigManager {
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      if (this.config.adjustmentHistory.length > MAX_HISTORY_ENTRIES) {
        this.config.adjustmentHistory = this.config.adjustmentHistory.slice(-MAX_HISTORY_ENTRIES);
      }
   ...
   */
  private getThresholdBounds(key: keyof SignalThresholds): { min: number; max: number } {
    switch (key) {
      case "minStrength":
