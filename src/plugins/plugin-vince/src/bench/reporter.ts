...
export function toMarkdown(report: VinceBenchReport): string {
  const { scoring, domainBreakdown, outcomeCorrelation, unmappedSignatures } = report;
  const lines: string[] = [
  ...
  for (const [name, d] of Object.entries(domainBreakdown)) {
    lines.push(`| ${name} | ${d.weight} | ${d.uniqueSignatures} | ${d.contribution.toFixed(2)} |`);
  }
  lines.push("");

  if (outcomeCorrelation) {
    lines.push("### Outcome Correlation", "");
    lines.push(`- High-score decisions win rate: ${(outcomeCorrelation.highScoreWinRate * 100).toFixed(1)}%`);
    lines.push(`- Low-score decisions win rate: ${(outcomeCorrelation.lowScoreWinRate * 100).toFixed(1)}%`);
    lines.push(`- Process-outcome correlation: ${outcomeCorrelation.correlation.toFixed(3)}`);
    lines.push("");
  }

  const sortedDomains = Object.entries(domainBreakdown).sort((a, b) => a[1].contribution - b[1].contribution);
  if (sortedDomains.length > 0) {
    lines.push("### Weakest Areas");
    sortedDomains.slice(0, 5).forEach(([name, d], i) => {
      lines.push(`${i + 1}. ${name}: ${d.contribution.toFixed(2)} — ${d.uniqueSignatures} unique signature(s)`);
    });
    lines.push("");
  }
 ...
 */
export function writeJsonReport(report: VinceBenchReport, outPath: string): void {
 ...
 */
export function writeMarkdownReport(report: VinceBenchReport, outPath: string): void {
  const dir = path.dirname(outPath);
  if (dir && !fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
 ...
 */
export function writeReports(report: VinceBenchReport, outDir: string): { jsonPath: string; mdPath: string } {
