...
function emitAvoidSignatures(reason: string, sigs: Signature[]): void {
  ...
  if (r.includes("circuit") || r.includes("pause")) sigs.push("avoid.circuit_breaker_blocked");
  if (r.includes("cooldown")) sigs.push("avoid.cooldown_blocked");
  if (r.includes("time filter") || r.includes("time filter blocked")) sigs.push("avoid.time_filter_blocked");
  if (r.includes("similar") || r.includes("similarity") || r.includes("suggest avoid"))
    sigs.push("avoid.similarity_avoid");
  if (r.includes("book") || r.includes("imbalance")) sigs.push("avoid.book_imbalance_rejected");
  if (r.includes("quality") || r.includes("threshold") || r.includes("strength") || r.includes("confidence"))
    sigs.push("avoid.low_quality_rejected");
  if (r.includes("regime") || r.includes("mismatch")) sigs.push("avoid.regime_mismatch_rejected");
...
function emitSignalQualitySignatures(
  ...
  if (signal.confidence >= CONFIDENCE_HIGH) sigs.push("signal.quality.high_confidence");
  if (signal.sourceCount >= SOURCE_COUNT_MULTI) sigs.push("signal.quality.multi_source_confirmed");
  if (signal.hasCascadeSignal) sigs.push("signal.quality.cascade_detected");
  if (signal.hasFundingExtreme) sigs.push("signal.quality.funding_extreme_used");
  if (signal.hasWhaleSignal) sigs.push("signal.quality.ml_filtered");
  if (signal.openWindowBoost >= OPEN_WINDOW_BOOST_MIN) sigs.push("entry.open_window_boost");
  const regimeAligned =
    (regime.marketRegime === "bullish" && signal.direction === "long") ||
    (regime.marketRegime === "bearish" && signal.direction === "short") ||
    (regime.marketRegime === "neutral" || regime.marketRegime === "volatile");
  if (regimeAligned && signal.direction !== "neutral") sigs.push("signal.quality.regime_aligned");
}
...
function emitPositionAndEntrySignatures(
  ...
  if (execution.positionSizePct > 0 && execution.positionSizePct <= 10) sigs.push("position.sizing.goal_aware");
...
function emitRiskSignatures(
  ...
  if (execution.leverage >= 1 && execution.leverage <= 10) sigs.push("risk.leverage_appropriate");
...
function emitTimingSignatures(session: FeatureRecord["session"], sigs: Signature[]): void {
...
function emitRegimeSignatures(regime: FeatureRecord["regime"], sigs: Signature[]): void {
  if (regime.marketRegime && regime.marketRegime !== "unknown") sigs.push("regime.detected");
  if (regime.volatilityRegime && regime.volatilityRegime !== "unknown")
    sigs.push("regime.volatility_adjusted");
  if (regime.marketRegime && ["bullish", "bearish", "neutral", "volatile"].includes(regime.marketRegime))
