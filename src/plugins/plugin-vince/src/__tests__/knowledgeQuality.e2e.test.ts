...
const VINCE_TEST_CASES: TestCase[] = [
  ...
  {
    domain: "OPTIONS",
    query: "Using the strike selection framework from your knowledge base, how should I adjust HYPE covered call strikes when funding is +0.03%? What does your knowledge say about APR thresholds and the Wheel Strategy implementation?",
    ...
    expectedTone: ["direct", "methodology-focused", "trader language", "framework-citing"],
    weight: 5,
  ...
  {
    domain: "PERPS",
    query: "According to your funding rate interpretation framework, what are the red flags when funding flips from negative to positive? Apply the methodology from your knowledge to explain entry timing.",
    expectedCapabilities: [
  ...
  {
    domain: "MEMES",
    query: "Using the trenches grinding methodology from your knowledge base, how should I evaluate a new Solana meme coin? Apply the H/E/F/S rating system if your knowledge includes it.",
    expectedCapabilities: [
  ...
  {
    domain: "AIRDROPS",
    query: "What does your knowledge base say about Tread.fi airdrop farming strategies? Apply any specific volume targets or tier thresholds from your knowledge to optimize farming approach.",
    expectedCapabilities: [
  ...
  {
    domain: "LIFESTYLE",
    query: "According to your Southwest France Palace methodology, which of the three Palaces should I choose for a gastronomy-focused one-night stay? Apply your 24-hour protocol and explain the off-season timing strategy. What makes Les Prés d'Eugénie different from Les Sources de Caudalie for this purpose?",
    expectedCapabilities: [
  ...
  {
    domain: "ART",
    query: "What methodology does your knowledge base provide for evaluating NFT floor thickness? Apply this framework to analyze CryptoPunks or Meridian liquidity specifically.",
    expectedCapabilities: [
...
describe("VINCE Knowledge Quality", () => {
  ...
  describe("A/B Comparison by Domain", () => {
  ...
  describe("Overall Knowledge Impact", () => {
    ...
    it("should show positive average improvement across all domains", async () => {
      ...
      // Calculate averages
      const avgBaseline = results.reduce((sum, r) => sum + r.baselineScore.overallScore, 0) / results.length;
      const avgEnhanced = results.reduce((sum, r) => sum + r.enhancedScore.overallScore, 0) / results.length;
      const avgImprovement = avgEnhanced - avgBaseline;
      const improvementPercent = avgBaseline > 0
        ? ((avgImprovement / avgBaseline) * 100).toFixed(1)
        : "N/A";
      
      // Calculate Knowledge Integration averages
      const avgKIBaseline = results.reduce((sum, r) => sum + r.baselineScore.knowledgeIntegration, 0) / results.length;
      const avgKIEnhanced = results.reduce((sum, r) => sum + r.enhancedScore.knowledgeIntegration, 0) / results.length;
      ...
      for (const r of results) {
        const kiDelta = r.enhancedScore.knowledgeIntegration - r.baselineScore.knowledgeIntegration;
        console.log(
      ...
      console.log(`Knowledge Integration Delta: +${Math.round(avgKIImprovement)} points`);
      ...
      if (pct >= 20) {
        ...
        console.log("LIMITED: Small improvement. Consider reviewing knowledge quality.");
      } else {
        console.log("NO IMPROVEMENT: Knowledge may not be matching queries well.");
      }

      // Best and worst domains
      const best = results.reduce((a, b) => a.improvement > b.improvement ? a : b);
      const worst = results.reduce((a, b) => a.improvement < b.improvement ? a : b);
    ...
    it("should have OPTIONS and PERPS as top performers (our deepest knowledge)", async () => {
      ...
      if (optionsResult && perpsResult) {
        ...
        console.log(`\nCore Domains Check:`);
        const optKIDelta = optionsResult.enhancedScore.knowledgeIntegration - optionsResult.baselineScore.knowledgeIntegration;
        const perpsKIDelta = perpsResult.enhancedScore.knowledgeIntegration - perpsResult.baselineScore.knowledgeIntegration;
        
        console.log(`  OPTIONS: Overall ${optionsResult.improvement >= 0 ? "PASS" : "FAIL"} (+${optionsResult.improvement}) | KI Delta: +${optKIDelta}`);
        console.log(`  PERPS: Overall ${perpsResult.improvement >= 0 ? "PASS" : "FAIL"} (+${perpsResult.improvement}) | KI Delta: +${perpsKIDelta}`);
        
        // At least one of our core domains should show improvement
        const coreImprovement = Math.max(optionsResult.improvement, perpsResult.improvement);
    ...
    it("should show Knowledge Integration improvement when knowledge is provided", async () => {
      ...
      const avgKIDelta = results.reduce((sum, r) => {
        return sum + (r.enhancedScore.knowledgeIntegration - r.baselineScore.knowledgeIntegration);
      }, 0) / results.length;
      
      console.log(`\nKnowledge Integration Check:`);
      console.log(`  Average KI Delta: +${Math.round(avgKIDelta)}`);
      console.log(`  This measures whether knowledge is actually being USED, not just present.`);
