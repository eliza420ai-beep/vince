...
describe.skipIf(skipNetworkTests)("HIP-3 API Verification", () => {
  const service = new VinceHIP3Service(mockRuntime);

  it(
    "should connect to HIP-3 API and fetch asset data",
    async () => {
      ...
      if (testResult.data) {
        ...
        console.log(
          `   Commodities: ${data.commodityCount} (GOLD, SILVER, OIL...)`,
        );
        ...
        if (data.samplePrices?.length > 0) {
          ...
          for (const p of data.samplePrices) {
            console.log(
              `     ${p.symbol}: $${p.price.toFixed(2)} (${p.category})`,
            );
          }
        }
        ...
        if (data.worstPerformer) {
          console.log(
            `   🔴 Worst Performer: ${data.worstPerformer.symbol} (${data.worstPerformer.change.toFixed(2)}%)`,
          );
        }
      }
  ...
  it(
    "should fetch full HIP-3 pulse with sector stats",
    async () => {
      ...
      if (pulse) {
        ...
        console.log(
          `     Gold: ${pulse.summary.goldVsBtc.goldChange.toFixed(2)}%`,
        );
        console.log(
          `     BTC: ${pulse.summary.goldVsBtc.btcChange.toFixed(2)}%`,
        );

        console.log("\n   Sector Stats:");
        console.log(`     Hottest Sector: ${pulse.sectorStats.hottestSector}`);
        console.log(
          `     Commodities: avg ${pulse.sectorStats.commodities.avgChange.toFixed(2)}% (${pulse.sectorStats.commodities.assetCount} assets)`,
        );
        console.log(
          `     Indices: avg ${pulse.sectorStats.indices.avgChange.toFixed(2)}% (${pulse.sectorStats.indices.assetCount} assets)`,
        );
        console.log(
          `     Stocks: avg ${pulse.sectorStats.stocks.avgChange.toFixed(2)}% (${pulse.sectorStats.stocks.assetCount} assets)`,
        );
        console.log(
          `     AI/Tech: avg ${pulse.sectorStats.aiPlays.avgChange.toFixed(2)}% (${pulse.sectorStats.aiPlays.assetCount} assets)`,
        );
        ...
        if (pulse.leaders.volumeLeaders.length > 0) {
          ...
          for (const leader of pulse.leaders.volumeLeaders.slice(0, 3)) {
            console.log(
              `     ${leader.symbol}: $${(leader.volume / 1e6).toFixed(2)}M`,
            );
          }
        }
  ...
  it(
    "should fetch individual asset prices",
    async () => {
  ...
  it("should report service status", () => {
    ...
    console.log(
      `   Last Update: ${status.lastUpdate > 0 ? new Date(status.lastUpdate).toISOString() : "Never"}`,
    );
