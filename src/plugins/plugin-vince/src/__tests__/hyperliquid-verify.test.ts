...
describe.skipIf(skipNetworkTests)("Hyperliquid API Verification", () => {
  const service = new HyperliquidFallbackService();

  it(
    "should connect to Hyperliquid API and fetch asset data",
    async () => {
      ...
      console.log(
        `   BTC 8h Funding: ${testResult.data.btcFunding8h !== null ? (testResult.data.btcFunding8h * 100).toFixed(4) + "%" : "N/A"}`,
      );
      console.log(
        `   ETH 8h Funding: ${testResult.data.ethFunding8h !== null ? (testResult.data.ethFunding8h * 100).toFixed(4) + "%" : "N/A"}`,
      );
      console.log(
        `   Sample assets: ${testResult.data.sampleAssets.join(", ")}\n`,
      );
    },
    TIMEOUT,
  );
  ...
  it(
    "should fetch options pulse with funding rates",
    async () => {
      ...
      if (optionsPulse?.assets) {
        ...
        if (btc) {
          const oi =
            btc.openInterest != null
              ? ` OI: ${fmtOi(btc.openInterest)} contracts`
              : "";
          const vol =
            btc.volume24h != null ? ` Vol24h: ${fmtVol(btc.volume24h)}` : "";
          console.log(
            `   BTC: ${btc.fundingAnnualized?.toFixed(2)}% annualized, ${btc.crowdingLevel} crowding, ${btc.squeezeRisk} squeeze risk${oi}${vol}`,
          );
        }
        if (eth) {
          const oi =
            eth.openInterest != null
              ? ` OI: ${fmtOi(eth.openInterest)} contracts`
              : "";
          const vol =
            eth.volume24h != null ? ` Vol24h: ${fmtVol(eth.volume24h)}` : "";
          console.log(
            `   ETH: ${eth.fundingAnnualized?.toFixed(2)}% annualized, ${eth.crowdingLevel} crowding, ${eth.squeezeRisk} squeeze risk${oi}${vol}`,
          );
        }
        if (sol) {
          const oi =
            sol.openInterest != null
              ? ` OI: ${fmtOi(sol.openInterest)} contracts`
              : "";
          const vol =
            sol.volume24h != null ? ` Vol24h: ${fmtVol(sol.volume24h)}` : "";
          console.log(
            `   SOL: ${sol.fundingAnnualized?.toFixed(2)}% annualized, ${sol.crowdingLevel} crowding${oi}${vol}`,
          );
        }
        if (hype) {
          const oi =
            hype.openInterest != null
              ? ` OI: ${fmtOi(hype.openInterest)} contracts`
              : "";
          const vol =
            hype.volume24h != null ? ` Vol24h: ${fmtVol(hype.volume24h)}` : "";
          console.log(
            `   HYPE: ${hype.fundingAnnualized?.toFixed(2)}% annualized, ${hype.crowdingLevel} crowding${oi}${vol}`,
          );
        }
      }
  ...
  it(
    ...
    async () => {
      ...
      if (crossVenue === null) {
        console.log(
          "⚠️  Cross-venue data returned null (this can happen due to API limits)",
        );
        return;
      }
      ...
      for (const asset of topAssets) {
        ...
        console.log(
          `   ${asset.coin}: HL=${hlRate}%, CEX=${cexRate}%${arbIndicator}`,
        );
      }
  ...
  it("should report rate limit status", () => {
    ...
    console.log(
      `   Backoff Until: ${status.backoffUntil > 0 ? new Date(status.backoffUntil).toISOString() : "N/A"}`,
    );
