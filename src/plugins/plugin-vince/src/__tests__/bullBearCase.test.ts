...
import { BullBearAnalyzer, getBullBearAnalyzer } from "../analysis/bullBearAnalyzer.ts";
import type { MarketDataSnapshot, DailyConclusion, MarketCase } from "../types/analysis.ts";
import {
  createMockRuntime,
...
describe("Bull Case Extraction", () => {
  ...
  it("should identify smart money accumulation as bullish", async () => {
    ...
    if (smartMoneyFactor) {
      ...
      expect(smartMoneyFactor.explanation).toContain("smart money");
    }
  });
...
describe("Bear Case Extraction", () => {
  ...
  it("should identify bearish signals from mock data", async () => {
    ...
    // Strength should be significant
    expect(bearCase.strength).toBeGreaterThan(30);
  });
...
describe("Conclusion Generation", () => {
  ...
  it("should produce bullish conclusion when bull case is stronger", async () => {
    ...
    expect(result.conclusion.bullCase.strength).toBeGreaterThan(result.conclusion.bearCase.strength);
  });
  ...
  it("should produce bearish conclusion when bear case is stronger", async () => {
    ...
    expect(result.conclusion.conviction).toBeGreaterThan(50);
    expect(result.conclusion.bearCase.strength).toBeGreaterThan(result.conclusion.bullCase.strength);
  });
  ...
  it("should produce neutral conclusion when signals conflict", async () => {
    ...
    // Either neutral direction or low conviction
    if (result.conclusion.direction === "neutral") {
      expect(result.conclusion.conviction).toBeLessThan(60);
    } else {
      // If not neutral, the strength difference should be relatively small
      expect(strengthDiff).toBeLessThan(50);
    }
  });
  ...
  it("should include key factors from both cases", async () => {
    ...
    const hasBullFactors = result.conclusion.keyFactors.some(f => f.includes("BULL"));
  ...
  it("should provide appropriate recommendation based on direction", async () => {
    ...
    expect(["accumulate", "hold"]).toContain(bullResult.conclusion.recommendation);
...
describe("Full Daily Analysis Integration", () => {
  it("should complete full analysis cycle with bullish data", async () => {
    ...
    expect(conclusion.bullCase.strength).toBeGreaterThan(conclusion.bearCase.strength);
  });
  ...
  it("should handle service errors gracefully", async () => {
    ...
    const runtime = {
      ...
      getService: vi.fn().mockImplementation(() => ({
        getFunding: () => { throw new Error("API Error"); },
        getLongShortRatio: () => { throw new Error("API Error"); },
        getFearGreed: () => { throw new Error("API Error"); },
        getOpenInterest: () => { throw new Error("API Error"); },
      })),
      getSetting: vi.fn().mockReturnValue(null),
    } as any;
