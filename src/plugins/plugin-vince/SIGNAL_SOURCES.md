...
| **XSentiment**                | VinceXSentimentService                            | X (Twitter) sentiment from search (confidence ≥40); staggered refresh every 7.5 min (one asset: BTC→SOL→ETH→HYPE); persistent cache `.elizadb/vince-paper-bot/x-sentiment-cache.json`; needs X_BEARER_TOKEN + VinceXResearchService. Optional cron: `bun run scripts/x-vibe-check.ts` (round-robin) or pass asset. |
...
| 5b  | **XSentiment**          | X (Twitter) search sentiment for BTC/ETH/SOL/HYPE. **Default weight: 0.5** (see below).                                                        | **VINCE_X_SENTIMENT_SERVICE**. Depends on **X_BEARER_TOKEN** and **VinceXResearchService**. Staggered refresh: one asset every 7.5 min to avoid X API rate limits; cache file `.elizadb/vince-paper-bot/x-sentiment-cache.json`. Optional external cron: `bun run scripts/x-vibe-check.ts` (see script header).      |
