...
| **PolymarketSentiment**       | VincePolymarketSentimentService                   | Prediction-market odds from Polymarket (BTC/ETH/SOL/macro/stocks); aggregates YES probability by tag bucket; confidence ≥40 (configurable via POLYMARKET_SENTIMENT_CONFIDENCE_FLOOR). Cache `.elizadb/vince-paper-bot/polymarket-sentiment-cache.json`; refresh every 15–30 min (POLYMARKET_SENTIMENT_REFRESH_MIN). Requires plugin-polymarket-discovery on same runtime (Vince loads it). Set POLYMARKET_SENTIMENT_ENABLED=false to disable. |
...
| **StandupSignal**             | Standup Day Report (plugin-inter-agent)           | Solus/standup call from `docs/standup/signals/YYYY-MM-DD-signals.json`; written after Day Report; one signal per asset (long/short). Set **VINCE_PAPER_STANDUP_SIGNALS_ENABLED=false** to disable. Default weight 0.6.                                                                                                                                                                                                                        |
...
- When **X_SENTIMENT_USE_X_RESEARCH_PLUGIN=true** and **plugin-x-research** is loaded (e.g. on ECHO or same process as VINCE), trading sentiment can come from **X_RESEARCH_TRADING_SENTIMENT_SERVICE** (topic-based search + quality-account weighting) instead of Vince’s keyword-only path. See `.env.example`: **X_SENTIMENT_USE_X_RESEARCH_PLUGIN**.
...
### Maximize X contribution (fewer “did not meet 40% or neutral”)

To get X sentiment to contribute more often (and reduce swarm dir=neutral no-trades):

1. **Enable soft tier:** Set **X_SENTIMENT_SOFT_TIER_ENABLED=true** so 25–39% confidence non-neutral sentiment still votes (with reduced weight).
2. **Lower floor for testing:** Set **X_SENTIMENT_CONFIDENCE_FLOOR=35** (default 40) so more signals pass the aggregator gate.
3. **Ensure cache is warm:** Use **X_SENTIMENT_PRIORITY_ASSETS=BTC,ETH** and **X_SENTIMENT_PRIORITY_STAGGER_MS=1800000** (30 min) so core assets refresh more often; or run the optional cron `bun run scripts/x-vibe-check.ts` to prime the cache.
4. **Debug why X didn’t vote:** Set **LOG_LEVEL=debug**; when X doesn’t contribute you’ll see one-line reasons (e.g. “no cache”, “cache stale”, “neutral”, “conf 32% < floor 40%”) in logs.

### Keyword lexicon and source quality (plugin-vince vs plugin-x-research)

- **Keywords:** Bullish/bearish/risk word lists live in **plugin-vince** `src/constants/sentimentKeywords.ts` (and optional **X_SENTIMENT_KEYWORDS_PATH** JSON). **plugin-x-research** has its own `src/constants/sentimentKeywords.ts` (and types in `types/sentiment.types.ts`) used for topic vibe and X_PULSE. The two codebases are separate; for consistent sentiment when both plugins are used, keep the two files in sync (periodic review or consider a shared lexicon package later).
- **Source quality:** **plugin-vince** uses **xSourceQualityReader** (per-handle precision from `data/x-source-quality.jsonl` → 0.5x / 1.0x / 1.5x multiplier in the aggregator). **plugin-x-research** uses **sourceReputation** / **qualityAccounts** (whale/alpha/retail tiers for weighting tweets). When **X_SENTIMENT_USE_X_RESEARCH_PLUGIN=true**, trading sentiment comes from plugin-x-research’s topic + tier-weighted model; when false, Vince’s keyword scoring and xSourceQualityReader apply. If you add account-level weighting to Vince (e.g. when using list or plugin-x-research data), align with the same quality tiers (whale/alpha/retail) so both plugins use consistent criteria.

### X list feed (curated list sentiment)

When **X_LIST_ID** is set (e.g. your curated list ID from x.com/i/lists/…), **VinceXResearchService** can fetch list metadata and list posts via **getListById**, **getListPosts**, and **getListMembers**. List posts are cached **15 minutes** (same TTL as search). **VinceXSentimentService.getListSentiment()** runs the same keyword/phrase/risk scoring over list posts and returns one aggregate sentiment (bullish/bearish/neutral + confidence + hasHighRiskEvent); result is cached in-memory for **15 minutes**. Set **X_SENTIMENT_LIST_ENABLED=false** to disable list sentiment (default: enabled when X_LIST_ID is set). List sentiment is exposed in: **CT Vibe** action (line "List (curated): …"), **leaderboard** News section (X vibe card shows a "List (curated)" row when present). **List sentiment now feeds the signal aggregator** as source **XListSentiment** when directional and confidence ≥ **X_LIST_SENTIMENT_CONFIDENCE_FLOOR** (default 35); weight in dynamicConfig is 0.4 (soft confirm).
...
| 5c  | **XListSentiment**      | Curated list (X_LIST_ID) sentiment; same keyword scoring as per-asset. **Default weight: 0.4**.                                              | **VINCE_X_SENTIMENT_SERVICE.getListSentiment()**. When **X_LIST_ID** is set and **X_SENTIMENT_LIST_ENABLED** is not false, list sentiment votes when directional and confidence ≥ **X_LIST_SENTIMENT_CONFIDENCE_FLOOR** (default 35). Cached 15 min.                                                                                                                         |
| 5c′ | **XNews**               | X News API (Grok summaries, finance.tickers). **Default weight: 0.35**.                                                                      | **X_NEWS_AGGREGATOR_SERVICE** (plugin-x-research). Set **X_NEWS_AS_AGGREGATOR_SOURCE=true** and load plugin-x-research. Aggregates sentiment from getCryptoNews(); contributes when confidence ≥ 25. Fallback when API unavailable.                                                                                                                                          |
| 5c″ | **XTrending**           | Soft factor when asset is trending or has volume spike on X. **Default weight: 0.2** (experimental).                                         | **X_TRENDS_SIGNAL_SERVICE** (plugin-x-research). When plugin loaded, getTrendingAssets() is called; if current asset is in the list, adds a weak neutral factor. Tie-breaker only.                                                                                                                                                                                           |
| 5d  | **StandupSignal**       | Standup/Solus call from Day Report (signals file). **Default weight: 0.6**.                                                                  | Written by **plugin-inter-agent** after Day Report to `docs/standup/signals/YYYY-MM-DD-signals.json` (or **STANDUP_DELIVERABLES_DIR**/signals). Aggregator reads for current asset; contributes when direction is long/short and confidence ≥ 25. Set **VINCE_PAPER_STANDUP_SIGNALS_ENABLED=false** to disable.                                                              |
| 5e  | **EchoXSignal**         | ECHO What's the Trade daily pick. **Default weight: 0.5**.                                                                                   | Written by **plugin-x-research** WTT task to `docs/standup/signals/YYYY-MM-DD-echo-x.json` (same dir as standup signals). Same-day file; one entry per primary (and optional alt) ticker. Set **VINCE_PAPER_ECHO_X_SIGNALS_ENABLED=false** to disable.                                                                                                                       |
| 5f  | **XBookmarks**          | Your saved X bookmarks → Pine pipeline (finance meta). **Default weight: 0.48**.                                                             | **plugin-x-research** `X_BOOKMARKS_PIPELINE` appends `data/x-bookmarks-pipeline/paper-signals.jsonl` after a successful Rust run; aggregator injects **latest row per asset** within **X_BOOKMARKS_PAPER_SIGNAL_TTL_HOURS** (default 72). Set **VINCE_PAPER_X_BOOKMARKS_ENABLED=false** to disable. See [docs/X-BOOKMARKS-PIPELINE.md](../../docs/X-BOOKMARKS-PIPELINE.md).  |
