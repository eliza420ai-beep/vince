...
## Endpoints we do *not* use yet (candidates for new signals)
...
- **Why it helps the algo:**
  - We already have **Binance** “extreme funding” via percentile (current vs last 30 samples). **Hyperliquid** today only uses *current* funding from `metaAndAssetCtxs` for bias/crowding, so “extreme” is a fixed annualized threshold.
...
- **Why it helps the algo:**
  - We already use **clearinghouseState** for Top Traders (whale positions). **userFunding** tells us whether those whales are *paying* or *receiving* funding over time.
