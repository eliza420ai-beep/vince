...
function regexExtract(text: string): Partial<PolymarketExtractedParams> {
  ...
  if (
    conditionIds &&
    conditionIds[0].length >= 42 &&
    conditionIds[0].length <= 66
  ) {
...
export async function extractPolymarketParams(
  ...
  try {
    ...
    const fromState = (composedState?.data?.actionParams ?? {}) as Record<
      string,
      unknown
    >;
    if (fromState && typeof fromState === "object") {
      if (fromState.query != null)
        params.query = String(fromState.query).trim();
      if (fromState.category != null)
        params.category = String(fromState.category).trim();
      if (fromState.conditionId != null)
        params.conditionId = String(fromState.conditionId).trim();
      if (fromState.condition_id != null)
        params.condition_id = String(fromState.condition_id).trim();
      if (fromState.tokenId != null)
        params.tokenId = String(fromState.tokenId).trim();
      if (fromState.walletAddress != null)
        params.walletAddress = String(fromState.walletAddress).trim();
      if (fromState.limit != null) {
        const n =
          typeof fromState.limit === "string"
            ? parseInt(fromState.limit, 10)
            : Number(fromState.limit);
        if (!isNaN(n)) params.limit = n;
      }
    }
  } catch (e) {
    logger.debug(
      "[extractPolymarketParams] composeState failed: " +
...
Reply with only the JSON object, no markdown or explanation.`;
      ...
      if (parsed.query != null && parsed.query !== null)
        params.query = String(parsed.query).trim();
      if (parsed.category != null && parsed.category !== null)
        params.category = String(parsed.category).trim();
      ...
      if (parsed.tokenId != null && parsed.tokenId !== null)
        params.tokenId = String(parsed.tokenId).trim();
      if (parsed.walletAddress != null && parsed.walletAddress !== null)
        params.walletAddress = String(parsed.walletAddress).trim();
      if (parsed.limit != null && parsed.limit !== null) {
        const n =
          typeof parsed.limit === "number"
            ? parsed.limit
            : parseInt(String(parsed.limit), 10);
        if (!isNaN(n)) params.limit = n;
      }
    } catch (e) {
      logger.debug(
        "[extractPolymarketParams] LLM extract failed: " +
          (e instanceof Error ? e.message : String(e)),
      );
    }
  }
