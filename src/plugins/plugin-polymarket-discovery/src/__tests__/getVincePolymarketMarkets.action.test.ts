...
const contextState = createMockState({
  recentMessagesData: [{ id: "1", content: { text: "polymarket markets" }, createdAt: 0 } as any],
});

describe("GET_VINCE_POLYMARKET_MARKETS validate", () => {
  it("returns false when not in context", async () => {
    const state = createMockState({
      recentMessagesData: [{ id: "1", content: { text: "weather" }, createdAt: 0 } as any],
    });
    ...
    const result = await getVincePolymarketMarketsAction.validate(runtime, message, state);
  ...
  it("returns false when service is null", async () => {
    ...
    const result = await getVincePolymarketMarketsAction.validate(runtime, message, contextState);
    expect(result).toBe(false);
  });
...
describe("GET_VINCE_POLYMARKET_MARKETS handler", () => {
  ...
  it("calls callback with market list when service returns one market", async () => {
    ...
    expect((result as any).data.markets[0].question).toBe(mockPolymarketMarket.question);
    expect((result as any).data.markets[0].condition_id).toBe(mockPolymarketMarket.conditionId);
    const lastCall = callback.calls[callback.calls.length - 1];
  ...
  it("uses group crypto when actionParams.group is crypto", async () => {
    const cryptoSlugs = [...new Set(VINCE_POLYMARKET_PREFERRED_LABELS.filter((e) => e.group === "crypto").map((e) => e.slug))];
