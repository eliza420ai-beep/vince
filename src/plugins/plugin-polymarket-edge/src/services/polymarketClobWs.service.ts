 ...
 *
 * Connects to market channel. Subscribes to token IDs from discovery.
 * Maintains local orderbook state per token. Ping every 10s.
 */
...
export class PolymarketClobWsService extends Service {
  static serviceType = "POLYMARKET_EDGE_CLOB_WS";
  capabilityDescription =
    "Real-time Polymarket CLOB orderbook per token for edge engine";
