...
import type { SupportedNetwork } from "../../../../constants/chains";
...
export class MorphoService extends Service {
  ...
  public async supply(
    ...
    try {
      ...
      console.log("Network:", params.chain);
  ...
  public async buildSupplyTx(
    ...
    const marketId = await this.getMarketId(params.market, params.chain);
  ...
  async getMarketData(
    ...
    if (market) {
      try {
        const marketId = await this.getMarketId(market, chain);
        const [summary, params] = await Promise.all([
          this.fetchMarketSummaryById(marketId, chain),
    ...
    try {
      const items = await this.fetchMarketsFromApi(this.getChainId(chain));
  ...
  async getUserPositionsByAddress(
    ...
    if (market?.trim()) {
      const marketId = await this.getMarketId(market, chain);
      const result = await this.buildUserPosition(address, marketId, chain, pc);
      return [result];
    }
  ...
  public async getVaultData(vault: string | undefined, chain: SupportedNetwork): Promise<MorphoVaultData[]> {
    ...
    // Single vault
    if (vault?.trim()) {
      const address = await this.resolveVaultAddress(vault, chain);
  ...
  public async getUserVaultPositionsByAddress(
    ...
    }>(Q_USER_VAULT_POSITIONS, { chainId: this.getChainId(chain), address });
  ...
  private async fetchMarketSummaryById(
    ...
    if (!m)
      throw new Error(
        `Market ${uniqueKey} not found on chainId ${this.getChainId(chain)}`,
      );
  ...
  private async buildUserPosition(
    ...
    const summary = await this.fetchMarketSummaryById(marketId, chain);
