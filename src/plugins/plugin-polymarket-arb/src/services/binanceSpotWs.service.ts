 ...
 *
 * Connects to BTCUSDT bookTicker stream (~200ms updates). No API key.
 * Maintains price state and rolling 1-minute price buffer for volatility.
 */
...
export class BinanceSpotWsService extends Service {
  static serviceType = "POLYMARKET_ARB_BINANCE_SPOT_WS";
  capabilityDescription =
    "Real-time Binance spot BTC price and rolling volatility for latency arb";
  ...
  /** Annualized volatility (decimal) from rolling 1-min returns; 0 if insufficient data */
  getVolatility(): number {
    ...
    const annualized = Math.sqrt(variance * (365 * 24 * 60)); // 1-min bars -> annual
    return Math.min(annualized, 2); // cap 200% vol
  }
