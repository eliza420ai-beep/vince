  ...
  handler: async (
    ...
    try {
      ...
      const prompt = isTaskBrief
        ...
        : `You are Sentinel. North star: 24/7 coding, self-improving, ML/ONNX obsessed, ART (elizaOS examples/art), clawdbot for knowledge research, best settings. Using the context below (internal-docs, sentinel-docs including ELIZAOS_BENCHMARKS for benchmark run commands, examples, elizaos-plugins), produce a short prioritized list of improvement suggestions. Categories: architecture/ops, ONNX/feature-store health, clawdbot spin-up, ART gems from examples, best-settings nudge, benchmarks alignment (use ELIZAOS_BENCHMARKS for context_bench, agentbench, solana, gauntlet, etc.), relevant plugins, tech debt. Number each item; one line per item with a short ref (doc or URL). No intro—just the numbered list.\n\nContext:\n${contextBlock}`;
