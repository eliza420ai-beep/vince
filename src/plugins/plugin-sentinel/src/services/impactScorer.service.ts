...
const CACHE_PATH = path.join(process.cwd(), ".openclaw-cache", "impact-scores.json");
...
const HIGH_IMPACT_KEYWORDS = [
  "revenue", "money", "profit", "alpha", "edge", "signal",
  "24/7", "market research", "vince push", "x research",
  "paper trading", "real trading", "live",
  "user", "customer", "adoption",
  "blocked", "critical", "urgent",
];

// Keywords that indicate lower priority
const LOW_PRIORITY_KEYWORDS = [
  "nice to have", "later", "maybe", "consider",
  "cleanup", "refactor", "style", "docs",
];
...
export function autoScore(item: WorkItem): ImpactScore {
  ...
  if (textLower.includes("24/7") || textLower.includes("market research") || textLower.includes("vince")) {
    revenueAlignment += 3;
    northStarAlignment += 3;
    impact += 2;
  }
  
  if (textLower.includes("trading") || textLower.includes("signal") || textLower.includes("alpha")) {
    revenueAlignment += 2;
    impact += 1;
  }
  
  if (textLower.includes("user") || textLower.includes("ux") || textLower.includes("dashboard")) {
    reach += 2;
  }
  
  if (textLower.includes("tech debt") || textLower.includes("refactor") || textLower.includes("cleanup")) {
    techDebtReduction += 3;
    impact -= 1;
  }
  
  if (textLower.includes("blocked") || textLower.includes("critical") || textLower.includes("urgent")) {
    impact += 2;
    confidence += 1;
  }
  
  // Estimate effort based on scope keywords
  if (textLower.includes("small") || textLower.includes("quick") || textLower.includes("minor")) {
    effort -= 2;
  }
  if (textLower.includes("large") || textLower.includes("major") || textLower.includes("rewrite")) {
    effort += 3;
  }
  ...
  northStarAlignment = Math.max(1, Math.min(10, Math.round(northStarAlignment)));
  techDebtReduction = Math.max(1, Math.min(10, Math.round(techDebtReduction)));
  
  // Calculate scores
  const riceScore = (reach * impact * confidence) / effort;
  const strategicScore = (revenueAlignment * 0.4 + northStarAlignment * 0.4 + techDebtReduction * 0.2);
 ...
 */
export function rankWorkItems(items: WorkItem[]): Array<WorkItem & { score: ImpactScore }> {
 ...
 */
export function getAcceptanceRates(): Record<string, { accepted: number; total: number; rate: number }> {
  const cache = loadCache();
  const rates: Record<string, { accepted: number; total: number; rate: number }> = {};
  ...
  for (const cat of Object.keys(rates)) {
    rates[cat].rate = rates[cat].total > 0 
...
export function getLearnings(): string[] {
  ...
  for (const [category, stats] of Object.entries(rates)) {
    if (stats.total >= 3) {
      if (stats.rate >= 80) {
        learnings.push(`✅ ${category}: ${stats.rate}% acceptance — keep suggesting`);
      } else if (stats.rate <= 30) {
        learnings.push(`⚠️ ${category}: ${stats.rate}% acceptance — reduce these suggestions`);
      }
    }
  }
  ...
  if (recentRejections.length > 0) {
    learnings.push(`📝 Recent rejection notes: ${recentRejections.map(r => r.outcomeNotes).join("; ")}`);
  }
...
export function formatScore(score: ImpactScore): string {
  const emoji = score.totalScore >= 50 ? "🔥" : score.totalScore >= 30 ? "🟢" : score.totalScore >= 15 ? "🟡" : "⚪";
