...
export const cdpAdvancedTradeCreateOrder: Action = {
  ...
  parameters: {
    product_id: { type: "string", description: "Trading pair e.g. BTC-USD", required: true },
    side: { type: "string", description: "BUY or SELL", required: true },
    quote_size: { type: "object", description: "Optional: { value: string, currency: string } e.g. { value: '100', currency: 'USD' }", required: false },
    base_size: { type: "object", description: "Optional: { value: string, currency: string } e.g. { value: '0.01', currency: 'BTC' }", required: false },
  },
  ...
  handler: async (
    ...
    const quoteSize = options?.quote_size as { value?: string; currency?: string } | undefined;
    const baseSize = options?.base_size as { value?: string; currency?: string } | undefined;

    if (!productId || !side || (side !== "BUY" && side !== "SELL")) {
      const text = "COINBASE_CREATE_ORDER needs product_id (e.g. BTC-USD) and side (BUY or SELL), plus quote_size or base_size.";
      await callback?.({ text });
      return { success: false, text };
    }
    if (!quoteSize && !baseSize) {
      const text = "Provide either quote_size (e.g. { value: '100', currency: 'USD' }) or base_size (e.g. { value: '0.01', currency: 'BTC' }).";
      await callback?.({ text });
      return { success: false, text };
    }
    ...
    try {
      const data = await advancedTradeRequest<CreateOrderResponse>("POST", PATH, config, body);
  ...
  examples: [
    [
      ...
      { name: "Otaku", content: { text: "Placing market buy on Coinbase…", actions: ["COINBASE_CREATE_ORDER"] } },
    ],
  ],
};
