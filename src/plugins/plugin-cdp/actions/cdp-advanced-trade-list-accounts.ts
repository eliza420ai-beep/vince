...
export const cdpAdvancedTradeListAccounts: Action = {
  ...
  handler: async (
    ...
    if (!config) {
      const text =
        "Coinbase Advanced Trade is not configured (set COINBASE_ADVANCED_TRADE_KEY_NAME and COINBASE_ADVANCED_TRADE_KEY_SECRET).";
      await callback?.({ text });
      return { success: false, text };
    }
    try {
      const path = "/api/v3/brokerage/accounts?limit=50";
      const data = await advancedTradeRequest<ListAccountsResponse>(
        "GET",
        path,
        config,
      );
  ...
  examples: [
    [
      { name: "user", content: { text: "What are my Coinbase accounts?" } },
      {
        name: "Otaku",
        content: {
          text: "Listing your Coinbase accounts…",
          actions: ["COINBASE_LIST_ACCOUNTS"],
        },
      },
    ],
  ],
