...
export const cdpWalletFetchWithPayment: Action = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory, state?: State) => {
    return validateCdpService(
      _runtime,
      "USER_WALLET_FETCH_WITH_PAYMENT",
      state,
      message,
    );
  },
  ...
  handler: async (
    ...
    try {
      ...
      const method = (params?.method?.trim()?.toUpperCase() || "GET") as
        | "GET"
        | "POST";
      ...
      logger.info(
        `[FETCH_WITH_PAYMENT] Getting viem wallet client for: ${accountName}`,
      );
      ...
      logger.info(
        `[FETCH_WITH_PAYMENT] Making ${method} request to ${url} with max payment ${maxPayment} USDC`,
      );
      callback?.({
        text: ` Making request to ${url}... (will handle payment if required)`,
      });
      ...
      if (paymentResponseHeader) {
        try {
          ...
          logger.info(
            `[FETCH_WITH_PAYMENT] Payment completed:`,
            JSON.stringify(paymentInfo),
          );
        } catch (err) {
          logger.warn(
            `[FETCH_WITH_PAYMENT] Failed to decode payment response:`,
      ...
      const { emoji: statusEmoji, prefix: statusPrefix } = getStatusIndicators(
        response.status,
        wasPaidRequest,
      );
    ...
    [
      {
        name: "{{user}}",
        content: {
          text: "fetch https://x402.example.com/premium-data with payment",
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: " Making request to https://x402.example.com/premium-data...",
    ...
    [
      {
        name: "{{user}}",
        content: {
          text: 'POST to https://x402.example.com/submit with payment using body {"key": "value"}',
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: " Making request to https://x402.example.com/submit...",
          action: "FETCH_WITH_PAYMENT",
    ...
    [
      {
        name: "{{user}}",
        content: {
          text: "try to fetch https://regular-api.com/free-endpoint with payment support",
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: " Making request to https://regular-api.com/free-endpoint...",
          action: "FETCH_WITH_PAYMENT",
