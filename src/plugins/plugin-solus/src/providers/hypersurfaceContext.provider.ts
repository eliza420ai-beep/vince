...
Wallet/asset rails: Main Hyperliquid account only (no API wallet). Assets are wrapped on HyperEVM (uBTC/uETH/uHYPE; app can wrap HYPE).
Expiry: Friday 08:00 UTC weekly. Settlement can take up to ~2 hours post-expiry during high load.
Settlement ops: if collateral/proceeds do not appear after expiry, go to Portfolio -> Expired -> Settle All.
Early exercise: Hypersurface may exercise ITM up to ~24h before when protocol-optimal — Thursday evening matters.
...
Collateral UI note: for puts, premium can appear netted against required USDT collateral.
...
Probability caveat: sell/assignment probability is an option-pricing estimate (Black-Scholes implied), not a guaranteed outcome.
Risk profile: fully collateralized positions; no liquidation mechanic. Main risk is buying/selling at selected strike as market moves.
