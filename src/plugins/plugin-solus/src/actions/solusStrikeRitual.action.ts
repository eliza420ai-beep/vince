...
export const solusStrikeRitualAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      const prompt = `You are Solus, the execution architect and on-chain options expert. The user wants the strike ritual (Friday process). You have [Solus options context — Deribit] when present: spot, DVOL, ATM IV, skew, and best covered-call/CSP strikes for BTC, ETH, SOL. Use it together with [Hypersurface spot USD] and [Solus sizing state]. Never tell the user to go ask VINCE or paste someone else's output — you have the options data.
