...
function createSolusRuntime(overrides?: {
  ...
  return {
    ...
    composeState:
      ...
      (async () => ({
        text: "Hypersurface: Friday 08:00 UTC. CC, CSP, wheel.",
      })),
    useModel: (overrides?.useModel ??
      (async () =>
        "Strike ritual: get VINCE options, pick asset, CC vs CSP, strike, invalidation.")) as any,
    getSetting: () => null,
    getService: () => null,
...
describe("solusStrikeRitualAction", () => {
  it("validate returns true for strike ritual phrases when Solus", async () => {
    ...
    for (const phrase of phrases) {
      const result = await solusStrikeRitualAction.validate!(
        runtime,
  ...
  it("handler calls callback with text", async () => {
    const runtime = createSolusRuntime({
      useModel: async () =>
        "Checklist: (1) VINCE options. (2) Pick asset. Next: paste his view.",
    });
...
describe("solusHypersurfaceExplainAction", () => {
  ...
  it("validate returns false when not Solus", async () => {
    ...
    const result = await solusHypersurfaceExplainAction.validate!(
      runtime,
      createMessage("explain hypersurface"),
    );
    expect(result).toBe(false);
  });
});
...
describe("solusPositionAssessAction", () => {
  it("validate returns true for position phrases when Solus", async () => {
    ...
    for (const phrase of phrases) {
      const result = await solusPositionAssessAction.validate!(
        runtime,
        createMessage(phrase),
      );
      expect(result).toBe(true);
  ...
  it("validate returns true for standup-style position phrases (underwater, assigned, our $70k puts)", async () => {
    const runtime = createSolusRuntime();
    const phrases = [
      "our $70K secured puts are underwater",
      "we might get assigned",
      "our $70k puts",
      "premium reduces cost basis",
    ];
    for (const phrase of phrases) {
...
describe("solusOptimalStrikeAction", () => {
  it("validate returns true for optimal strike phrases when Solus", async () => {
    ...
    for (const phrase of phrases) {
      const result = await solusOptimalStrikeAction.validate!(
        runtime,
        createMessage(phrase),
      );
      expect(result).toBe(true);
