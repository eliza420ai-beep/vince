...
import { createMockMessage, createMockState, createMockRuntime, withEnv } from "./test-utils";
...
describe("openclawContextProvider.get", () => {
  ...
  it("returns context when message contains openclaw", async () => {
    ...
    const result = await openclawContextProvider.get(runtime, message, createMockState());
    ...
    expect(
      (result.text ?? "").includes("releases") || (result.text ?? "").includes("github.com/openclaw")
    ).toBe(true);
  });
  ...
  it("sets openclawGatewayConfigured true when OPENCLAW_GATEWAY_URL is set", async () => {
    await withEnv({ OPENCLAW_GATEWAY_URL: "http://127.0.0.1:18789" }, async () => {
      const message = createMockMessage("openclaw");
      const result = await openclawContextProvider.get(runtime, message, createMockState());
      expect(result.values!.openclawGatewayConfigured).toBe(true);
      expect((result.text ?? "").toLowerCase()).toContain("gateway url is set");
    });
  });
});
