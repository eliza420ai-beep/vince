...
async function getUserIdFromMessage(
  runtime: ActionEventPayload["runtime"],
  messageId?: UUID,
  roomId?: UUID,
  entityId?: UUID,
): Promise<UUID | null> {
  ...
  const resolveActualUserId = async (id: UUID): Promise<UUID | null> => {
    try {
      ...
      if (
        entity.metadata?.author_id &&
        typeof entity.metadata.author_id === "string"
      ) {
...
async function recordSwapPoints(payload: ActionEventPayload): Promise<boolean> {
  try {
    const gamificationService = payload.runtime.getService(
      "gamification",
    ...
    if (actionResult.values?.swapSuccess === false) {
      logger.debug(
        "[Gamification] Skipping points for swap marked as unsuccessful",
      );
      return false;
    }
    ...
    const volumeUsd = validateVolumeUsd(
      actionResult?.values?.volumeUsd ?? actionResult?.values?.valueUsd,
    );

    const userId = await getUserIdFromMessage(
      payload.runtime,
      payload.messageId,
      payload.roomId,
    );
...
async function recordBridgePoints(
  ...
  try {
    ...
    const chain =
      actionResult?.values?.destinationChain ?? actionResult?.values?.toChain;
...
async function recordTransferPoints(
  payload: ActionEventPayload,
): Promise<boolean> {
  try {
    const gamificationService = payload.runtime.getService(
...
async function recordChatPoints(payload: RunEventPayload): Promise<void> {
  try {
    ...
    try {
      ...
      logger.debug(
        { error },
        "[Gamification] Could not fetch message for chat points",
      );
      return;
    }
...
async function recordAgentActionPoints(
  payload: ActionEventPayload,
): Promise<void> {
  try {
    const gamificationService = payload.runtime.getService(
    ...
    logger.error(
      { error },
      "[Gamification] Error recording agent action points",
    );
  }
}

async function recordAccountCreationPoints(
  ...
  try {
    ...
    const referralService = payload.runtime.getService(
      "referral",
    ) as ReferralService;
    if (referralService) {
      try {
        ...
        if (referredBy && typeof referredBy === "string") {
          logger.info(
            `[Gamification] Processing referral code ${referredBy} for user ${userId}`,
          );
        ...
        logger.error(
          { error: err },
          "[Gamification] Error processing referral in account creation",
        );
      }
    }
  } catch (error) {
    logger.error(
      { error },
      "[Gamification] Error recording account creation points",
    );
  }
}
