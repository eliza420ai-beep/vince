...
function checkRepeatedChars(text: string): { valid: boolean; ratio: number } {
  ...
  const repeatedLength = matches.reduce(
    (sum: number, m: string) => sum + m.length,
    0,
  );
  ...
  return {
    valid:
      ratio < CONTENT_QUALITY_CONFIG.MAX_CHAR_REPETITION_RATIO &&
 ...
 */
function checkRepeatedWords(text: string): {
  valid: boolean;
  uniqueRatio: number;
} {
  ...
  return {
    valid:
      uniqueRatio >= CONTENT_QUALITY_CONFIG.MIN_UNIQUE_WORD_RATIO &&
 ...
 */
function checkKeyboardMash(text: string): {
  valid: boolean;
  matchRatio: number;
} {
  let matchedLength = 0;
...
function checkUrlSpam(text: string): { valid: boolean; ratio: number } {
  ...
  const urlLength = urls.reduce(
    (sum: number, url: string) => sum + url.length,
    0,
  );
