...
export class ReferralService extends Service {
  ...
  async getOrCreateCode(
    userId: UUID,
  ): Promise<{ code: string; stats: ReferralStats }> {
    const db = this.getDb();
    ...
    logger.info(
      `[ReferralService] Created new referral code for user ${userId.substring(0, 8)}...`,
    );
      ...
      .where(
        and(
          ...
          eq(
            ...
            GamificationEventType.REFERRAL_ACTIVATION,
          ),
        ),
      );
    ...
    const referralEvents = await db
      ...
      .where(
        and(
          ...
          eq(
            gamificationEventsTable.actionType,
            GamificationEventType.REFERRAL_SIGNUP,
          ),
        ),
      );

    const totalPointsEarned = referralEvents.reduce(
      (sum: number, event: { points: number }) => sum + event.points,
      0,
    );
  ...
  async processReferralSignup(
    ...
    const gamificationService = this.runtime.getService(
      "gamification",
  ...
  async processReferralActivation(userId: UUID): Promise<void> {
    ...
    if (existingActivation) {
      ...
      const alreadyActivated = allActivations.some(
        (event: any) => event.metadata?.activatedUserId === userId,
      );
  ...
  private async generateUniqueReferralCode(userId: UUID): Promise<string> {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      logger.warn(
        `[ReferralService] Referral code collision detected (attempt ${attempt + 1}/${MAX_ATTEMPTS}), regenerating...`,
      );
    }
