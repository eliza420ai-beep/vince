  ...
  init: async (_config: Record<string, string>, _runtime: IAgentRuntime) => {
    ...
    coreLogger.error = function (message: any, ...args: any[]) {
      ...
      if (
        ...
        if (process.env.LOG_LEVEL === "debug") {
          originalDebug(
            "[LogFilter] Suppressed error (MESSAGE-BUS/central_messages/OpenSea):",
    ...
    console.warn = function (...args: unknown[]) {
      ...
      if (shouldSuppressConsoleWarn(msg)) {
        if (process.env.LOG_LEVEL === "debug") {
          originalConsoleWarn(
            "[LogFilter] Suppressed console.warn:",
            msg.substring(0, 80) + "...",
          );
        }
        return;
    ...
    isPatched = true;
    coreLogger.info(
      "[LogFilter] ✅ Logger filter active - suppressing MCP schemas, MESSAGE-BUS/SQL/embedding noise, AI SDK warnings",
    );
  },
};
