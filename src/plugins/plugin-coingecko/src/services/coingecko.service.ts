...
export class CoinGeckoService extends Service {
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    this.proApiKey =
      (runtime.getSetting("COINGECKO_API_KEY") as string) ||
      process.env.COINGECKO_API_KEY;
    const usePublic =
      runtime.getSetting("COINGECKO_USE_PUBLIC_URL") ??
      process.env.COINGECKO_USE_PUBLIC_URL;
    this.forcePublicUrl =
      usePublic === true ||
      String(usePublic).toLowerCase() === "true" ||
      String(usePublic) === "1";
    ...
    try {
      const res = await fetch(url, {
        ...
        headers: {
          ...
          ...(isPro && this.proApiKey
            ? { "x-cg-pro-api-key": this.proApiKey }
            : {}),
  ...
  async getTokenMetadata(
    ...
    for (const rawId of normalizedIds) {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        logger.warn(
          `[CoinGecko] resolveCandidates failed for ${q}: ${message}`,
        );
      }
      ...
      const enrichedCandidates = await this.enrichCandidateMetadata(
        baseUrl,
        candidates,
      );
      const primaryCandidate = enrichedCandidates.find((candidate) =>
      ...
      try {
        ...
        const tokenId =
          typeof data.id === "string" && data.id.length > 0
            ? data.id
            : platform;
        return formatCoinMetadata(tokenId, data, platform) as Record<
          string,
          unknown
        >;
      } catch {
  ...
  private async handleContractLookup(
    ...
    try {
      const metadata = await this.fetchByContractAddress(
        baseUrl,
        address,
        platforms,
      );
      ...
      const message = error instanceof Error ? error.message : String(error);
      logger.warn(
        `[CoinGecko] Contract lookup failed for ${address}: ${message}`,
      );
      return {
        id: address,
        success: false,
  ...
  private async loadCoinsIndex(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        logger.debug(
          `[CoinGecko] Loading coins index (attempt ${attempt}/${maxAttempts}): ${url}`,
        );
        const res = await fetch(url, {
          method: "GET",
          headers: {
        ...
        if (!res.ok) {
          ...
          const isDemoKeyError =
            ...
            (body?.error_code === 10011 ||
              String(body?.status?.error_message ?? "").includes(
                "Demo API key",
              ));
          if (isDemoKeyError && !this.usePublicUrlOverride) {
            this.usePublicUrlOverride = true;
            logger.info(
              "[CoinGecko] Demo API key detected; switching to public URL (api.coingecko.com). Set COINGECKO_USE_PUBLIC_URL=true to avoid this.",
            );
            await this.loadCoinsIndex();
            return;
          }
          if (
            ...
            attempt === 1 &&
            isPro &&
            !this.usePublicUrlOverride &&
            this.proApiKey
          ) {
            this.usePublicUrlOverride = true;
            logger.info(
              "[CoinGecko] Pro API rate limited (429); retrying once with public URL (api.coingecko.com).",
            );
            await this.loadCoinsIndex();
            return;
          }
          ...
          if (res.status === 429) {
            ...
            if (attempt === maxAttempts) {
              throw new Error(
                `Failed to load coins list ${res.status}: ${res.statusText}${body ? ` - ${JSON.stringify(body)}` : ""}`,
              );
            }
        ...
        logger.info(
          `[CoinGecko] Coins index loaded: ${this.coinsCache.length} entries`,
        );
        return;
      } catch (e) {
        clearTimeout(timeout);
        ...
        if (isLast) {
          logger.error(
            `[CoinGecko] Failed to load coins index after ${maxAttempts} attempts: ${msg}`,
          );
          this.coinsCache = [];
          this.idSet.clear();
          this.symbolToIds.clear();
        ...
        if (is429) {
          ...
          logger.warn(
            `[CoinGecko] Coins index fetch failed (attempt ${attempt}): ${msg}. Retrying in ${backoff}ms...`,
          );
        }
  ...
  private async resolveCandidates(
    ...
    if (candidateIds.size > 0) {
      ...
      if (ranked.length > 0) {
        logger.debug(
          { query, ranked: ranked.map((candidate) => candidate.coinId) },
          "[CoinGecko] Ranked candidate matches",
        );
        return ranked;
      }
    ...
    const nearSymbols = Array.from(this.symbolToIds.keys())
      .filter(
        (key) => key === query || key.startsWith(query) || key.includes(query),
      )
      .slice(0, 10);
    ...
    logger.debug(
      { query, nearSymbols, nearNames },
      "[CoinGecko] No direct candidate matches",
    );
    return [];
  }

  private async rankByMarkets(
    ids: string[],
  ): Promise<TokenMetadataCandidate[]> {
    if (ids.length === 0) {
      return [];
    ...
    const candidates = ids.map<TokenMetadataCandidate>((id) => {
      ...
      const marketCap =
        typeof marketRow?.market_cap === "number" ? marketRow.market_cap : null;
      const totalVolume =
        typeof marketRow?.total_volume === "number"
          ? marketRow.total_volume
          : null;
      const marketCapRank =
        typeof marketRow?.market_cap_rank === "number"
          ? marketRow.market_cap_rank
          : null;
      return {
        id,
        coinId: id,
    ...
    const volumeValues = candidates.map(
      (candidate) => candidate.totalVolume ?? 0,
    );
    ...
    const scored = candidates.map((candidate) => {
      ...
      const volumeScore =
        maxVolume > 0 ? (candidate.totalVolume ?? 0) / maxVolume : 0;
      const rankScore =
        candidate.marketCapRank && candidate.marketCapRank > 0
          ? 1 / candidate.marketCapRank
          : 0;
    ...
    return scored
      .map((entry) => ({
        ...entry.candidate,
        confidence:
          totalScore > 0 ? entry.rawScore / totalScore : fallbackConfidence,
      }))
  ...
  private async enrichCandidateMetadata(
    ...
    for (const candidate of primaryCandidates) {
      ...
      if (detail) {
        const formatted = formatCoinMetadata(
          candidate.coinId,
          detail,
  ...
  async getTrendingTokens(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.warn(
          `[CoinGecko] trending tokens request failed for ${network}: ${msg}`,
        );
        throw new Error(msg);
      }
      ...
      const included = Array.isArray((data as any).included)
        ? (data as any).included
        : [];
      ...
      const trendingTokens = pools.map((pool: any, index: number) => {
        ...
        return {
          ...
          volume_24h_usd: attrs.volume_usd?.h24
            ...
            : null,
          price_change_percentage_24h: priceChangePct.h24
            ? parseFloat(priceChangePct.h24)
            : null,
          network,
          address: baseToken?.address || null,
          rank: index + 1,
          liquidity_usd: attrs.reserve_in_usd
            ? parseFloat(attrs.reserve_in_usd)
            : null,
          fdv_usd: attrs.fdv_usd ? parseFloat(attrs.fdv_usd) : null,
          price_change_percentage_1h: priceChangePct.h1
            ? parseFloat(priceChangePct.h1)
            : null,
      ...
      logger.error(
        `[CoinGecko] getTrendingTokens failed for ${network}: ${msg}`,
      );
      throw err;
    }
  }
  ...
  async getNFTCollectionStats(collectionIdentifier: string): Promise<any> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.warn(
          `[CoinGecko] NFT collection stats request failed for ${collectionId}: ${msg}`,
        );
        throw new Error(msg);
      }
      ...
      return {
        ...
        image:
          data.image?.large || data.image?.small || data.image?.thumb || null,
        ...
        floor_price_24h_change_percentage:
          data.floor_price_24h_percentage_change?.usd || null,
        ...
        volume_24h_change_percentage:
          data.volume_24h_percentage_change?.usd || null,
        ...
        number_of_unique_addresses_24h_percentage_change:
          data.number_of_unique_addresses_24h_percentage_change || null,
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logger.error(
        `[CoinGecko] getNFTCollectionStats failed for ${collectionId}: ${msg}`,
      );
      throw err;
    }
  }
  ...
  async getTrendingSearch(): Promise<any> {
    ...
    try {
      ...
      const trendingCoins = (Array.isArray(data.coins) ? data.coins : []).map(
        (coinWrapper: any, index: number) => {
      ...
      const trendingNFTs = (Array.isArray(data.nfts) ? data.nfts : []).map(
        (nft: any, index: number) => {
          ...
          return {
            ...
            floor_price_change_24h:
              nftData.floor_price_in_usd_24h_percentage_change || null,
      ...
      const trendingCategories = (
        ...
        return {
          ...
          market_cap_change_24h:
            categoryData.market_cap_change_percentage_24h?.usd ||
            category.market_cap_change_24h ||
            null,
          total_market_cap: categoryData.market_cap
            ? categoryData.market_cap.toString()
            : category.total_market_cap?.toString() || null,
          total_volume: categoryData.total_volume
            ? categoryData.total_volume.toString()
            : category.total_volume?.toString() || null,
        };
      });
  ...
  async getTokenPriceChart(
    ...
    if (isContractAddress) {
      ...
      try {
        ...
        logger.warn(
          `[CoinGecko] Failed to fetch token info for ${tokenIdentifier}`,
        );
      }
    } else {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.warn(
          `[CoinGecko] price chart request failed for ${tokenIdentifier}: ${msg}`,
        );
        throw new Error(msg);
      }
      ...
      const dataPoints = filteredPrices.map(
      ...
      const marketCapDataPoints = filteredMarketCaps.map(
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logger.error(
        `[CoinGecko] getTokenPriceChart failed for ${tokenIdentifier}: ${msg}`,
      );
      throw err;
    }
  }
  ...
  async getHistoricalPrice(
    ...
    if (!/^\d{2}-\d{2}-\d{4}$/.test(date)) {
      throw new Error(
        `Invalid date format: ${date}. Expected format: dd-mm-yyyy (e.g., 01-01-2024)`,
      );
    }
    ...
    if (isContractAddress) {
      ...
      logger.debug(
        `[CoinGecko] Resolving contract address ${tokenIdentifier} on ${chain}`,
      );
      ...
      try {
        ...
        if (!res.ok) {
          const body = await safeReadJson(res);
          throw new Error(
            `Failed to resolve contract address: ${res.status} ${res.statusText}${body ? ` - ${JSON.stringify(body)}` : ""}`,
          );
        }
        ...
        logger.debug(
          `[CoinGecko] Resolved ${tokenIdentifier} to coin_id: ${coinId}`,
        );
      } catch (err) {
        clearTimeout(timeout);
        const msg = err instanceof Error ? err.message : String(err);
        throw new Error(
          `Failed to resolve contract address to coin_id: ${msg}`,
        );
      }
    } else {
      ...
      tokenSymbol = tokenIdentifier.toUpperCase();
      logger.debug(
        `[CoinGecko] Using coin_id: ${coinId} for token: ${tokenIdentifier}`,
      );
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.warn(
          `[CoinGecko] historical price request failed for ${coinId} on ${date}: ${msg}`,
        );
        throw new Error(msg);
      }
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logger.error(
        `[CoinGecko] getHistoricalPrice failed for ${coinId} on ${date}: ${msg}`,
      );
      throw err;
    }
  }
  ...
  async getCategoriesWithMarketData(
    order:
      ...
      | "market_cap_change_24h_asc" = "market_cap_desc",
  ): Promise<
    Array<{
      id: string;
      name: string;
