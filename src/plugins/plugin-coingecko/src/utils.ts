...
export async function fetchCoinsList(
  ...
  if (!r.ok) {
    ...
    throw new Error(
      `Failed to load coins list ${r.status}: ${r.statusText}${body ? ` - ${JSON.stringify(body)}` : ""}`,
    );
  }
  ...
  try {
    ...
    return rows
      ...
      .sort((a, b) => {
        ...
        const rankA =
          typeof a.market_cap_rank === "number" && a.market_cap_rank > 0
            ? a.market_cap_rank
            : 10_000_000;
        const rankB =
          typeof b.market_cap_rank === "number" && b.market_cap_rank > 0
            ? b.market_cap_rank
            : 10_000_000;
        return rankA - rankB;
      })
    ...
    clearTimeout(timeout);
    logger.warn(
      `[CoinGecko:rankIdsByMarkets] fetch failed: ${e instanceof Error ? e.message : String(e)}`,
    );
    return ids;
  }
}
...
function choosePlatform(
  ...
  if (
    preferredPlatform &&
    (detail[preferredPlatform] || platforms[preferredPlatform])
  ) {
    const d = detail[preferredPlatform];
    const address: string | null =
      (d?.contract_address as string) ||
      (platforms?.[preferredPlatform] as string) ||
      null;
    const decimals: number | null =
      typeof d?.decimal_place === "number" ? d.decimal_place : null;
  ...
  const d = detail[pick];
  const address: string | null =
    (d?.contract_address as string) || (platforms?.[pick] as string) || null;
  const decimals: number | null =
    typeof d?.decimal_place === "number" ? d.decimal_place : null;
...
export function formatCoinMetadata(
  requestId: string,
  data: any,
  preferredPlatform?: string | null,
): any {
  ...
  const { platformKey, address, decimals } = choosePlatform(
    data,
    preferredPlatform,
  );
  ...
  return {
    ...
    attributes: {
      ...
      total_supply:
        typeof md.total_supply === "number" ||
        typeof md.total_supply === "string"
          ? String(md.total_supply)
          : null,
      ...
      total_reserve_in_usd:
        usd(md, ["total_value_locked", "usd"])?.toString() ?? null,
      volume_usd: {
    ...
    market_data: {
      ...
      fully_diluted_valuation:
        usd(md, ["fully_diluted_valuation", "usd"]) ?? null,
      total_volume: usd(md, ["total_volume", "usd"]) ?? null,
      high_24h: usd(md, ["high_24h", "usd"]) ?? null,
      low_24h: usd(md, ["low_24h", "usd"]) ?? null,
      price_change_24h_in_currency:
        usd(md, ["price_change_24h_in_currency", "usd"]) ?? null,
      price_change_percentage_1h_in_currency:
        usd(md, ["price_change_percentage_1h_in_currency", "usd"]) ?? null,
      price_change_percentage_24h_in_currency:
        usd(md, ["price_change_percentage_24h_in_currency", "usd"]) ?? null,
      price_change_percentage_7d_in_currency:
        usd(md, ["price_change_percentage_7d_in_currency", "usd"]) ?? null,
      price_change_percentage_14d_in_currency:
        usd(md, ["price_change_percentage_14d_in_currency", "usd"]) ?? null,
      price_change_percentage_30d_in_currency:
        usd(md, ["price_change_percentage_30d_in_currency", "usd"]) ?? null,
      price_change_percentage_60d_in_currency:
        usd(md, ["price_change_percentage_60d_in_currency", "usd"]) ?? null,
      price_change_percentage_200d_in_currency:
        usd(md, ["price_change_percentage_200d_in_currency", "usd"]) ?? null,
      price_change_percentage_1y_in_currency:
        usd(md, ["price_change_percentage_1y_in_currency", "usd"]) ?? null,
      market_cap_change_24h_in_currency:
        usd(md, ["market_cap_change_24h_in_currency", "usd"]) ?? null,
      market_cap_change_percentage_24h_in_currency:
        usd(md, ["market_cap_change_percentage_24h_in_currency", "usd"]) ??
        null,
      total_supply:
        typeof md.total_supply === "number"
          ? md.total_supply
          : Number(md.total_supply) || null,
      max_supply:
        typeof md.max_supply === "number"
          ? md.max_supply
          : Number(md.max_supply) || null,
      max_supply_infinite: md.max_supply == null,
      circulating_supply:
        typeof md.circulating_supply === "number"
          ? md.circulating_supply
          : Number(md.circulating_supply) || null,
      last_updated:
        (md.last_updated as string) || (data as any)?.last_updated || null,
    },
