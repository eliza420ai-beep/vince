...
async function getNexusTokenBalances(
  ...
  logger.info(
    `[BICONOMY_AUTO_WITHDRAW] Scanning tokens on ${chainName} (${chainId}) for Nexus ${nexusAddress}`,
  );
  ...
  try {
    ...
    logger.info(
      `[BICONOMY_AUTO_WITHDRAW] Native ${nativeSymbol} balance on ${chainName}: ${formatUnits(nativeBalance, 18)} (${nativeBalance.toString()} wei)`,
    );

    if (nativeBalance > 0n) {
      logger.info(
        `[BICONOMY_AUTO_WITHDRAW] ✅ Adding native ${nativeSymbol} to withdrawal list`,
      );
      balances.push({
        chainId,
        chainName,
      ...
      logger.info(
        `[BICONOMY_AUTO_WITHDRAW] Native ${nativeSymbol} balance is zero, skipping`,
      );
    }
  } catch (error) {
    logger.error(
      `[BICONOMY_AUTO_WITHDRAW] ❌ Error checking native balance on ${chainName}: ${(error as Error).message}`,
    );
  }
  ...
  for (const symbol of priorityTokens) {
    ...
    try {
      ...
      if (balance > 0n) {
        ...
        logger.info(
          `[BICONOMY_AUTO_WITHDRAW] Found ${symbol.toUpperCase()} balance: ${balanceFormatted}`,
        );
      ...
      logger.debug(
        `[BICONOMY_AUTO_WITHDRAW] Error checking ${symbol}: ${(error as Error).message}`,
      );
    }
  }
...
function filterSpamTokens(balances: TokenBalance[]): TokenBalance[] {
  ...
  const filtered = balances.filter((balance) => {
    const threshold =
      SPAM_THRESHOLDS[balance.tokenSymbol] ||
      parseUnits("0.00001", balance.decimals);
  ...
  logger.info(
    `[BICONOMY_AUTO_WITHDRAW] Filtered ${balances.length - filtered.length} spam tokens, ${filtered.length} legitimate tokens remaining`,
  );
  return filtered;
}
...
async function getTokenUsdPrice(
  ...
  try {
    const { getTokenMetadata } =
      await import("../../../plugin-relay/src/utils/token-resolver");
    const metadata = await getTokenMetadata(address, chainName);
    return metadata?.usdPrice || null;
  } catch (error) {
    logger.debug(
      `[BICONOMY_AUTO_WITHDRAW] Error getting USD price: ${(error as Error).message}`,
    );
    return null;
  }
}
...
async function getMinAmountInUsd(
  ...
  if (!usdPrice || usdPrice <= 0) {
    logger.warn(
      `[BICONOMY_AUTO_WITHDRAW] No USD price found for ${symbol}, using fallback minimum`,
    );
  ...
  logger.debug(
    `[BICONOMY_AUTO_WITHDRAW] ${symbol} price: $${usdPrice}, need ${tokensNeeded} tokens for $${MIN_USD_VALUE}`,
  );
...
async function findFundingTokenWithBalance(
  ...
  for (const symbol of priorityOrder) {
    ...
    try {
      ...
      const decimals = await getTokenDecimals(address, chainName);
      const { minAmount, usdValue } = await getMinAmountInUsd(
        symbol,
        address,
        decimals,
        chainName,
      ...
      logger.debug(
        `[BICONOMY_AUTO_WITHDRAW] ${chainName}: Error checking ${symbol}: ${(error as Error).message}`,
      );
    }
  }
...
No parameters needed - fully automatic!`,
  ...
  handler: async (
    ...
    try {
      ...
      const fundingAmountPerChain = (params?.fundingAmount?.trim() ||
        "2") as string;
      ...
      callback?.({
        text: "🔍 Starting auto-withdrawal scan across all chains...",
      });
      ...
      logger.info(
        `[BICONOMY_AUTO_WITHDRAW] User address: ${userAddress}, Withdraw to: ${withdrawAddress}`,
      );
      ...
      for (const [chainName, chainId] of supportedChains) {
        try {
          ...
          if (!nexusAddress) {
            logger.info(
              `[BICONOMY_AUTO_WITHDRAW] No Nexus account found on ${chainName}, skipping`,
            );
            continue;
          }

          logger.info(
            `[BICONOMY_AUTO_WITHDRAW] Found Nexus account on ${chainName}: ${nexusAddress}`,
          );
          ...
          const balances = await getNexusTokenBalances(
            publicClient,
            nexusAddress,
            chainId,
            chainName,
          ...
          if (balances.length > 0) {
            ...
            callback?.({
              text: `✅ Found ${balances.length} token(s) on ${chainName}`,
            });
          } else {
          ...
          logger.error(
            `[BICONOMY_AUTO_WITHDRAW] Error scanning ${chainName}: ${(error as Error).message}`,
          );
          callback?.({
            text: `⚠️ Error scanning ${chainName}: ${(error as Error).message}`,
          });
        }
      }
      ...
      callback?.({
        text: `📊 Found ${totalTokensFound} total token(s) across ${chainsWithBalances.length} chain(s)`,
      });
      ...
      callback?.({
        text: `✅ ${legitimateBalances.length} legitimate token(s) to withdraw`,
      });
      ...
      logger.info(
        `[BICONOMY_AUTO_WITHDRAW] Executing ${chainIds.length} separate withdrawal(s) - one per chain with same-chain funding`,
      );

      const successfulWithdrawals: Array<{
        chainName: string;
        tokens: TokenBalance[];
        supertxHash: string;
        explorerUrl: string;
      ...
      for (const chainId of chainIds) {
        ...
        callback?.({
          text: `🔄 Processing ${balances.length} token(s) on ${chainName}...`,
        });
        ...
        try {
          ...
          const fundingTokenInfo = await findFundingTokenWithBalance(
          ...
          callback?.({
            text: `✅ Using ${fundingAmountPerChain} ${fundingTokenInfo.symbol.toUpperCase()} from ${chainName}`,
          });
          ...
          const fundingDecimals = await getTokenDecimals(
            fundingTokenInfo.address,
            chainName,
          );
          const fundingAmountWei = parseUnits(
            fundingAmountPerChain,
            fundingDecimals,
          );
          ...
          logger.info(
            `[BICONOMY_AUTO_WITHDRAW] Executing withdrawal on ${chainName} (${balances.length} tokens)`,
          );
          ...
          if (result.success && result.supertxHash) {
            const explorerUrl = biconomyService.getExplorerUrl(
              result.supertxHash,
            );
            ...
            callback?.({
              text: `✅ ${chainName} withdrawal complete! [Track](${explorerUrl})`,
            });
          } else {
            const errorMsg = result.error || "Unknown error";
            failedWithdrawals.push({ chainName, error: errorMsg });
            callback?.({
              text: `❌ ${chainName} withdrawal failed: ${errorMsg}`,
            });
          }
        } catch (error) {
          const errorMsg = (error as Error).message;
          logger.error(
            `[BICONOMY_AUTO_WITHDRAW] Error on ${chainName}: ${errorMsg}`,
          );
          failedWithdrawals.push({ chainName, error: errorMsg });
      ...
      if (successfulWithdrawals.length > 0) {
        ...
        for (const withdrawal of successfulWithdrawals) {
          const tokenList = withdrawal.tokens
            .map(
              (t) => `${formatUnits(t.balance, t.decimals)} ${t.tokenSymbol}`,
            )
            .join(", ");
      ...
      const isPartialSuccess =
        successfulWithdrawals.length > 0 && failedWithdrawals.length > 0;
      const isFullSuccess =
        successfulWithdrawals.length > 0 && failedWithdrawals.length === 0;
      ...
      if (isFullSuccess) {
        callback?.({
          text: summaryText,
          actions: ["BICONOMY_AUTO_WITHDRAW"],
  ...
  examples: [
    [
      {
        name: "{{user}}",
        content: {
          text: "Withdraw all my tokens from Biconomy Nexus accounts",
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: "Scanning all chains for tokens in your Nexus accounts...",
          action: "BICONOMY_AUTO_WITHDRAW",
        },
      },
    ],
    [
      {
        name: "{{user}}",
        content: {
          text: "Clean up my Biconomy smart wallets and withdraw everything",
        },
      },
      {
        name: "{{agent}}",
        content: {
          text: "Auto-withdrawing all tokens from Nexus accounts across all chains...",
          action: "BICONOMY_AUTO_WITHDRAW",
        },
      },
    ],
    [
      ...
      },
      {
        name: "{{agent}}",
        content: {
          text: "Sweeping all Nexus accounts and withdrawing tokens...",
          action: "BICONOMY_AUTO_WITHDRAW",
        },
      },
    ],
