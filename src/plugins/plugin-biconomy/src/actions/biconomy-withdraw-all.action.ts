...
async function findFundingTokenWithBalance(
  ...
  for (const symbol of priorityOrder) {
    ...
    try {
      logger.debug(
        `[BICONOMY_WITHDRAW] Checking ${symbol.toUpperCase()} balance at ${address}`,
      );
      ...
      if (balance >= minAmount) {
        logger.info(
          `[BICONOMY_WITHDRAW] Found funding token: ${symbol.toUpperCase()} with balance ${balance.toString()}`,
        );
        return { symbol, address: address as `0x${string}` };
      } else {
        logger.debug(
          `[BICONOMY_WITHDRAW] ${symbol.toUpperCase()} balance too low: ${balance.toString()}`,
        );
      }
    } catch (error) {
      logger.debug(
        `[BICONOMY_WITHDRAW] Error checking ${symbol}: ${(error as Error).message}`,
      );
      continue;
    }
  }
  ...
  logger.warn(
    `[BICONOMY_WITHDRAW] No suitable funding token found with balance on ${chainName}`,
  );
  return null;
}
...
Parameters: chain, token, amount (required for native tokens), fundingAmount (optional), withdrawAddress (optional).`,
  ...
  handler: async (
    ...
    try {
      ...
      callback?.({
        text: `🔍 Finding suitable funding token from your ${chainName} balance...`,
      });
      ...
      if (!fundingTokenInfo) {
        callback?.({
          text: `❌ No suitable funding token found in your EOA on ${chainName}. Please have at least 0.5 USDC, USDT, or DAI available for orchestration fees.`,
        });
        return {
          text: `❌ No suitable funding token found in EOA`,
          success: false,
          error: "no_funding_token",
        };
      }

      const { symbol: fundingTokenSymbol, address: fundingTokenAddress } =
        fundingTokenInfo;
      logger.info(
        `[BICONOMY_WITHDRAW] Using funding token: ${fundingTokenSymbol.toUpperCase()} at ${fundingTokenAddress}`,
      );

      const isNativeWithdrawal = isNativeToken(tokenAddress);
      const tokenLabel = isNativeWithdrawal ? "ETH" : tokenParam.toUpperCase();

      callback?.({
        text: `🔄 Creating ${isNativeWithdrawal ? "native token" : "ERC20"} withdrawal instruction for ${tokenLabel} on ${chainName}...`,
      });
      ...
      if (isNativeWithdrawal) {
        ...
        logger.info(
          `[BICONOMY_WITHDRAW] Using native token withdrawal for ${tokenLabel} on ${chainName}`,
        );

        // Amount is REQUIRED for native tokens
        if (!amountParam) {
          callback?.({
            text: `❌ Amount is required for native token (${tokenLabel}) withdrawals. Please specify the amount (e.g., amount: "0.1" for 0.1 ${tokenLabel})`,
          });
          return {
            text: `❌ Amount required for native token withdrawal`,
            success: false,
        ...
        if (!Number.isFinite(amountFloat) || amountFloat <= 0) {
          callback?.({
            text: `❌ Invalid amount: ${amountParam}. Please provide a positive number.`,
          });
          return {
            text: `❌ Invalid amount`,
            success: false,
        ...
        callback?.({
          text: `💰 Withdrawing ${amountParam} ${tokenLabel} from Nexus`,
        });
      ...
      callback?.({
        text: `🔄 Getting quote (funding: ${fundingAmount} ${fundingTokenSymbol.toUpperCase()} from your EOA)...`,
      });
  ...
  examples: [
    ...
    [
      ...
      },
      {
        name: "{{agent}}",
        content: {
          text: "Withdrawing 0.05 native ETH from Nexus Smart Account on Arbitrum...",
          action: "BICONOMY_WITHDRAW",
        },
      },
    ],
    [
      ...
      },
      {
        name: "{{agent}}",
        content: {
          text: "Withdrawing 10 native POL from Nexus Smart Account on Polygon...",
          action: "BICONOMY_WITHDRAW",
        },
      },
    ],
