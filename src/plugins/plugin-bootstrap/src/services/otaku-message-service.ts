 ...
 */
async function ensureAllAgentsInRoom(
  runtime: IAgentRuntime,
  roomId: UUID,
): Promise<void> {
  const eliza = (
    runtime as unknown as {
      elizaOS?: {
        getAgents?(): { agentId: string; character?: { name?: string } }[];
      };
    }
  ).elizaOS;
  ...
  for (const a of agents) {
    ...
    try {
      ...
      logger.debug(
        { agentId, roomId, err },
        "[OtakuMessageService] ensureAllAgentsInRoom: ensureConnection skip",
      );
    }
  }
}
...
async function ensureMessageSendersInRoom(
  ...
  for (const entityId of entityIds) {
    try {
      ...
      logger.debug(
        { entityId, roomId, err },
        "[OtakuMessageService] ensureMessageSendersInRoom skip",
      );
    }
  }
}
...
export class OtakuMessageService implements IMessageService {
    ...
    try {
      ...
      if (src) {
        ...
        if (channelIdVal !== undefined)
          (message as { channel_id?: string }).channel_id =
            typeof channelIdVal === "string" ? channelIdVal : undefined;
        if (messageServerIdVal !== undefined)
          (message as { message_server_id?: string }).message_server_id =
            typeof messageServerIdVal === "string"
              ? messageServerIdVal
              : undefined;
      }
      ...
      const isJobRequest =
        (message.content.metadata as Record<string, unknown>)?.isJobMessage ===
        true;
      ...
      } as Parameters<typeof runtime.emitEvent>[1] & {
        runId: UUID;
        messageId: UUID;
        source: string;
      });
    ...
    try {
      ...
      if (message.entityId === runtime.agentId) {
        runtime.logger.debug(
          `[OtakuMessageService] Skipping message from self`,
        );
      ...
      if (message.id) {
        ...
        if (existingMemory) {
          runtime.logger.debug(
            "[OtakuMessageService] Memory already exists, skipping creation",
          );
      ...
      await ensureMessageSendersInRoom(
        runtime,
        message.roomId,
        recentForEnsure,
      );
      ...
      if (!shouldRespondToMessage) {
        runtime.logger.debug(
          `[OtakuMessageService] Not responding based on evaluation`,
        );
      ...
      if (useMultiStep) {
        runtime.logger.debug(
          "[OtakuMessageService] Using multi-step processing",
        );
        ...
        runtime.logger.debug(
          "[OtakuMessageService] Using single-shot processing",
        );
        ...
        if (mode === "simple") {
          ...
          await runtime.processActions(
            ...
            async (content) => {
              runtime.logger.debug(
                { content },
                "[OtakuMessageService] action callback",
              );
      ...
      await runtime.evaluate(
        ...
        async (content) => {
          runtime.logger.debug(
            { content },
            "[OtakuMessageService] evaluate callback",
          );
          if (responseContent) {
      ...
      if (message.metadata && "entityName" in message.metadata) {
        entityName = (message.metadata as Record<string, unknown>)
      ...
      const actionsData = (state.data?.providers as Record<string, unknown>)
        ?.ACTIONS as Record<string, unknown> | undefined;
      const actionsDataContent = (actionsData?.data as Record<string, unknown>)
        ?.actionsData as Action[] | undefined;
      const availableActions = actionsDataContent?.map(
        (a: Action) => a.name,
      ) || [-1];
  ...
  private async runMultiStepCore(
      ...
      try {
        ...
        if (parameters) {
          if (typeof parameters === "string") {
            ...
            runtime.logger.debug(
              `[MultiStep] Using parameters object: ${JSON.stringify(actionParams)}`,
            );
          }
        }
        ...
        if (action) {
          ...
          runtime.logger.debug(
            `[MultiStep] Refreshing state after action ${action}`,
          );
      ...
      try {
        ...
        if (summary?.text) {
          ...
          if (summaryAttempt < maxSummaryRetries) {
            ...
            runtime.logger.debug(
              `[MultiStep] Retrying summary in ${delay}ms...`,
            );
            await new Promise((resolve) => setTimeout(resolve, delay));
          }
        }
        ...
        runtime.logger.error(
          `[MultiStep] Error during summary generation attempt ${summaryAttempt}`,
        );
        if (summaryAttempt >= maxSummaryRetries) {
          runtime.logger.warn(
    ...
    if (summary?.text) {
      ...
      runtime.logger.warn(
        `[MultiStep] No valid summary generated, using fallback`,
      );
      responseContent = {
        actions: ["MULTI_STEP_SUMMARY"],
  ...
  private async runSingleShotCore(
    ...
    if (!currentState.values?.actionNames) {
      runtime.logger.warn(
        "[OtakuMessageService] actionNames missing from state",
      );
    }
    ...
    if (!parsedResponse) {
      runtime.logger.error(
        "[OtakuMessageService] All single-shot attempts failed",
      );
      return {
        responseContent: null,
        responseMessages: [],
    ...
    const responseContent: Content = {
      ...
      providers: parsedResponse.providers
        ? String(parsedResponse.providers)
            .split(",")
            .map((p: string) => p.trim())
            .filter(Boolean)
        : [],
    ...
    runtime.logger.debug(
      {
        text: responseContent.text?.substring(0, 100),
        actions: responseContent.actions,
      },
  ...
  shouldRespond(
    ...
    const isStandupRoom =
      roomName.length > 0 &&
      standupParts.some((part: string) =>
        roomName.toLowerCase().includes(part),
      );
    ...
    if (isStandupRoom) {
      ...
      const metadata = message.content?.metadata as
        | Record<string, unknown>
        | undefined;
      const isFromBot = metadata?.isBot === true || metadata?.fromBot === true;
      ...
      const isDirectlyCalled =
        ...
        new RegExp(
          `\\b${nameLower.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} you're up`,
          "i",
        ).test(messageText) ||
        new RegExp(
          `\\b${nameLower.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}—`,
  ...
  async deleteMessage(runtime: IAgentRuntime, message: Memory): Promise<void> {
    try {
      if (!message.id) {
        runtime.logger.error(
          "[OtakuMessageService] Cannot delete memory: message ID is missing",
        );
        return;
      }
      ...
      runtime.logger.error(
        { error },
        "[OtakuMessageService] Error in deleteMessage",
      );
      throw error;
    }
  }
  ...
  async clearChannel(
    ...
    try {
      ...
      runtime.logger.error(
        { error },
        "[OtakuMessageService] Error in clearChannel",
      );
      throw error;
    }
  }
