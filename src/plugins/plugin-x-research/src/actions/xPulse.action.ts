...
export const xPulseAction: Action = {
  name: "X_PULSE",
  description:
    'Get an ALOHA-style briefing on X/Twitter sentiment and alpha for crypto topics. Use when asked about "what\'s CT saying", "X vibe", "twitter sentiment", "crypto twitter", etc.',
  ...
  handler: async (
    ...
    try {
      ...
      const text = (message.content?.text ?? "").toLowerCase();
      const quick =
        process.env.X_PULSE_QUICK === "true" || /quick|fast/.test(text);
      const qualityOnly =
        process.env.X_PULSE_QUALITY === "true" ||
        /quality|curated|whale take/.test(text);
      ...
      const highPriorityTopicIds = ALL_TOPICS.filter(
        (t) => t.priority === "high",
      ).map((t) => t.id);
      ...
      const emptyTopics = requestedTopicIds.filter(
        (id) => (topicResults.get(id)?.length ?? 0) === 0,
      );
      ...
      if (qualityOnly && allTweets.length > 0) {
        allTweets = allTweets.filter(
          (t) =>
            t.computed?.qualityTier && t.computed.qualityTier !== "standard",
        );
      }
      ...
      if (narrative) {
        ...
        if (qualityOnly)
          prefix += "_Quality/curated mode — whale & alpha accounts only._\n\n";
        briefing =
          prefix +
          narrative +
...
async function generateBriefing(
  ...
  if (qualityOnly)
    output += `_Quality/curated mode — whale & alpha accounts only._\n\n`;

  if (mandoContext?.vibeCheck) {
    output += `**Today's news:** ${mandoContext.vibeCheck}\n\n`;
    if (mandoContext.headlines?.length > 0) {
      const topHeadlines = mandoContext.headlines
        .slice(0, 5)
        .map((h) => (h.length > 60 ? h.slice(0, 57) + "..." : h));
  ...
  const topTopics = Object.entries(sentiment.byTopic as Record<string, any>)
    .sort(
      (a: [string, any], b: [string, any]) =>
        b[1].breakdown.totalAnalyzed - a[1].breakdown.totalAnalyzed,
    )
    .slice(0, 4);
