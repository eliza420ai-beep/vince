...
const ALOHA_STYLE_RULES = `STYLE RULES:
...
async function fetchXContext(runtime: IAgentRuntime): Promise<string> {
  ...
  try {
    ...
    if (deduped.length === 0) return 'No recent X posts found for OpenClaw/AGI in the last 24h.';
...
async function fetchWebContext(runtime: IAgentRuntime): Promise<string> {
  ...
  if (snippets.length === 0) return 'No web snippets (set TAVILY_API_KEY for web-sourced report).';
...
export const clawtermDayReportAction: Action = {
  ...
  similes: ['OPENCLAW_WHATS_HOT', 'CLAWTERM_WHATS_HOT', 'DAY_REPORT', 'OPENCLAW_DAY_REPORT'],
  examples: [
    [
      {
        user: '{{user1}}',
