...
describe("X_PULSE Action", () => {
  describe("validate", () => {
    it('should trigger on "x pulse"', async () => {
      const result = await xPulseAction.validate!(
        mockRuntime,
    ...
    it('should trigger on "what\'s ct saying"', async () => {
      const result = await xPulseAction.validate!(
        mockRuntime,
        createMemory("what's ct saying about btc"),
      );
      expect(result).toBe(true);
    });
    ...
    it('should trigger on "crypto twitter"', async () => {
      const result = await xPulseAction.validate!(
        mockRuntime,
        createMemory("crypto twitter sentiment"),
      );
      expect(result).toBe(true);
    });
    ...
    it("should not trigger on unrelated message", async () => {
      const result = await xPulseAction.validate!(
        mockRuntime,
        createMemory("what is the price of btc"),
      );
...
describe("X_VIBE Action", () => {
  describe("validate", () => {
    it('should trigger on "vibe on ETH"', async () => {
      const result = await xVibeAction.validate!(
        mockRuntime,
        createMemory("what's the vibe on ETH"),
      );
      expect(result).toBe(true);
    });
    ...
    it('should trigger on "BTC sentiment"', async () => {
      const result = await xVibeAction.validate!(
        mockRuntime,
        createMemory("BTC sentiment check"),
      );
      expect(result).toBe(true);
    });
    ...
    it("should trigger on topic + mood keyword", async () => {
      const result = await xVibeAction.validate!(
        mockRuntime,
        createMemory("feeling about solana"),
      );
      expect(result).toBe(true);
    });
    ...
    it("should not trigger without vibe keyword", async () => {
      const result = await xVibeAction.validate!(
        mockRuntime,
        createMemory("tell me about BTC"),
      );
      expect(result).toBe(false);
    });
  });
