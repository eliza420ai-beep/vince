...
describe('XNewsService', () => {
  ...
  describe('searchNews', () => {
    it('should search news and enrich results', async () => {
      const mockNews: XNewsItem[] = [
        createMockNews('BTC ETF Sees Record Inflows', 'BlackRock ETF hits $1B', ['BTC']),
        createMockNews('Ethereum L2 Growth', 'Layer 2 TVL reaches ATH', ['ETH']),
      ];
  ...
  describe('relevance scoring', () => {
    it('should score higher for focus tickers', async () => {
      ...
      expect(btcResult?.relevanceScore).toBeGreaterThan(randomResult?.relevanceScore ?? 0);
    });
  });
...
function createMockNews(name: string, summary: string, tickers: string[]): XNewsItem {
  return {
