...
describe('clawtermDayReportAction', () => {
  describe('validate', () => {
    it('returns true for "what\'s hot today?"', async () => {
      const { clawtermDayReportAction } = await import('../actions/clawtermDayReport.action');
      const result = await clawtermDayReportAction.validate!({} as IAgentRuntime, createMemory("What's hot today?"));
      expect(result).toBe(true);
    });

    it('returns true for "full day report"', async () => {
      const { clawtermDayReportAction } = await import('../actions/clawtermDayReport.action');
      const result = await clawtermDayReportAction.validate!({} as IAgentRuntime, createMemory('full day report'));
      expect(result).toBe(true);
    });

    it('returns true for "openclaw news today"', async () => {
      const { clawtermDayReportAction } = await import('../actions/clawtermDayReport.action');
      const result = await clawtermDayReportAction.validate!({} as IAgentRuntime, createMemory('openclaw news today'));
      expect(result).toBe(true);
    });

    it('returns false for unrelated message', async () => {
      const { clawtermDayReportAction } = await import('../actions/clawtermDayReport.action');
      const result = await clawtermDayReportAction.validate!({} as IAgentRuntime, createMemory('gateway status'));
  ...
  describe('handler', () => {
    ...
    it('prompt includes ALOHA-style rules', async () => {
      let capturedPrompt: string | undefined;
      const runtime = {
        getSetting: vi.fn((key: string) => (key === 'X_BEARER_TOKEN' || key === 'TAVILY_API_KEY' ? 'x' : null)),
        useModel: vi.fn(async (_type: unknown, params: { prompt: string }) => {
          capturedPrompt = params.prompt;
          return MOCK_REPORT;
        }),
      ...
      const callback = vi.fn();
      const { clawtermDayReportAction } = await import('../actions/clawtermDayReport.action');
      await clawtermDayReportAction.handler!(runtime, createMemory("what's hot today"), {} as State, {}, callback as HandlerCallback);
    ...
    it('prompt includes NO AI SLOP ban list', async () => {
      ...
      const callback = vi.fn();
      const { clawtermDayReportAction } = await import('../actions/clawtermDayReport.action');
      await clawtermDayReportAction.handler!(runtime, createMemory('day report'), {} as State, {}, callback as HandlerCallback);
    ...
    it('prompt contains X and web context from mocks', async () => {
      ...
      const callback = vi.fn();
      const { clawtermDayReportAction } = await import('../actions/clawtermDayReport.action');
      await clawtermDayReportAction.handler!(runtime, createMemory('what\'s hot'), {} as State, {}, callback as HandlerCallback);
