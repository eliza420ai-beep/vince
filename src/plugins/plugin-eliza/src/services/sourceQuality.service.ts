...
const TRUSTED_DOMAINS: Record<
  string,
  { score: number; trust: SourceRecord["trustLevel"]; tags: string[] }
> = {
  ...
  "vitalik.eth.limo": {
    score: 92,
    trust: "verified",
    tags: ["crypto", "ethereum", "thought-leader"],
  },
  "a]16z.com": {
    score: 85,
    trust: "trusted",
    tags: ["crypto", "vc", "research"],
  },
  "paradigm.xyz": {
    score: 88,
    trust: "trusted",
...
function calculateQualityScore(source: SourceRecord): number {
  ...
  const { contentCount, citationCount, userUpvotes, userDownvotes } =
    source.metrics;
...
export function loadQualityDB(): QualityDatabase {
  try {
    if (fs.existsSync(QUALITY_DB_PATH)) {
      return JSON.parse(
        fs.readFileSync(QUALITY_DB_PATH, "utf-8"),
      ) as QualityDatabase;
    }
  } catch (e) {
...
export function saveQualityDB(db: QualityDatabase): void {
  ...
  db.stats = {
    ...
    avgQuality:
      db.sources.length > 0
        ? Math.round(
            db.sources.reduce((sum, s) => sum + s.qualityScore, 0) /
              db.sources.length,
          )
        : 0,
    verifiedSources: db.sources.filter((s) => s.trustLevel === "verified")
      .length,
    untrustedSources: db.sources.filter((s) => s.trustLevel === "untrusted")
      .length,
  };
...
export function recordIngestion(
  ...
  logger.debug(
    `[SourceQuality] Recorded ingestion: ${fileId} from ${domain} (quality: ${source.qualityScore})`,
  );
...
export function recordFeedback(
  ...
  saveQualityDB(db);
  logger.info(
    `[SourceQuality] Recorded ${type} for ${sourceId}, new score: ${source.qualityScore}`,
  );
}
...
export function getQualityReport(): {
  ...
  // Identify concerns
  const untrusted = db.sources.filter(
    (s) => s.trustLevel === "untrusted" || s.trustLevel === "cautious",
  );
  if (untrusted.length > 0) {
    concerns.push(`${untrusted.length} sources with low trust ratings`);
    for (const s of untrusted.slice(0, 3)) {
      concerns.push(
        `  - ${s.name}: ${s.qualityScore}/100 (${s.metrics.contentCount} files)`,
      );
    }
  }
  ...
  if (highVolumeUnverified.length > 0) {
    recommendations.push(
      `Review ${highVolumeUnverified.length} high-volume neutral sources for trust upgrade`,
    );
  }
  ...
  if (stale.length > 0) {
    recommendations.push(
      `${stale.length} sources haven't been updated in 6+ months`,
    );
  }
  ...
  if (db.stats.verifiedSources < 3) {
    recommendations.push(
      "Add more content from verified sources (arxiv, ethereum.org, etc.)",
    );
  }
 ...
 */
export function scanAndUpdateQuality(): {
  updated: number;
  newSources: number;
} {
  const db = loadQualityDB();
  ...
  logger.info(
    `[SourceQuality] Scan complete: ${updated} files, ${newSources} new sources`,
  );
