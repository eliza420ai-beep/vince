...
export function analyzeCategory(categoryPath: string): ContentHealth | null {
  ...
  if (oldest.age > 60) {
    healthScore -= 10;
    issues.push(
      `Very old file: ${oldest.name} (${Math.round(oldest.age)} days)`,
    );
  }
  ...
  return {
    ...
    newestFile: {
      name: newest.name,
      age: newest.age === Infinity ? 0 : newest.age,
    },
...
export function runMonitorScan(): {
  ...
  const avgHealth =
    healthReports.length > 0
      ? Math.round(
          healthReports.reduce((sum, h) => sum + h.healthScore, 0) /
...
export function getCurrentSuggestions(): Suggestion[] {
  ...
  if (
    !state.lastRun ||
    Date.now() - new Date(state.lastRun).getTime() > 3600000
  ) {
