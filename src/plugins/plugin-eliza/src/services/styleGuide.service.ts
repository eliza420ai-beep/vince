...
export interface StyleGuide {
  ...
  targetTone: ("authoritative" | "conversational" | "educational" | "provocative")[];
...
export interface StyleViolation {
  rule: string;
  type: "terminology" | "capitalization" | "tone" | "prohibited" | "required" | "formatting" | "custom";
  severity: "error" | "warning" | "info";
  message: string;
  line?: number;
  column?: number;
...
const DEFAULT_STYLE_GUIDE: StyleGuide = {
  ...
  voiceDescription: "Authoritative yet accessible. We explain complex crypto concepts clearly without dumbing them down. Confident but not arrogant. Forward-thinking but grounded in fundamentals.",
  
  targetTone: ["authoritative", "educational", "conversational"],
  
  terminology: [
    { preferred: "crypto", avoid: ["cryptocurrency", "cryptocurrencies"], context: "general use" },
    { preferred: "DeFi", avoid: ["defi", "DEFI", "Defi"], caseSensitive: true },
    { preferred: "NFT", avoid: ["nft", "Nft", "Non-fungible token"], caseSensitive: true },
    { preferred: "web3", avoid: ["Web3", "WEB3", "Web 3", "web 3"], caseSensitive: true },
    { preferred: "onchain", avoid: ["on-chain", "on chain", "On-chain"], caseSensitive: true },
    { preferred: "Bitcoin", avoid: ["bitcoin", "BTC"], context: "when referring to the network", caseSensitive: true },
    { preferred: "Ethereum", avoid: ["ethereum", "ETH"], context: "when referring to the network", caseSensitive: true },
    { preferred: "Layer 2", avoid: ["L2", "layer 2", "layer-2"], context: "first mention" },
  ...
  toneMarkers: [
    // Casual/weak markers
    { pattern: "obviously", tone: "casual", suggestion: "Remove - can sound condescending" },
    { pattern: "simply", tone: "casual", suggestion: "Remove - nothing is 'simple' to everyone" },
    { pattern: "just", tone: "casual", suggestion: "Consider removing - often unnecessary" },
    { pattern: "basically", tone: "casual", suggestion: "Remove - weakens authority" },
    { pattern: "actually", tone: "casual", suggestion: "Remove - often unnecessary" },
    { pattern: "I think", tone: "casual", suggestion: "Be more assertive: state the opinion directly" },
    { pattern: "I believe", tone: "casual", suggestion: "Be more assertive: state the opinion directly" },
    { pattern: "In my opinion", tone: "casual", suggestion: "Be more assertive: state the opinion directly" },
    { pattern: "hopefully", tone: "casual", suggestion: "Be confident - state what it does, not what you hope" },
    { pattern: "might help", tone: "casual", suggestion: "Be confident - state the benefit directly" },
    // Promotional/hype markers
    { pattern: "!!", tone: "promotional", suggestion: "Single exclamation max" },
    { pattern: "🚀", tone: "promotional", suggestion: "Avoid rocket emoji - too promotional" },
    { pattern: "WAGMI", tone: "promotional", suggestion: "Avoid crypto slang in essays" },
    { pattern: "NGMI", tone: "promotional", suggestion: "Avoid crypto slang in essays" },
    { pattern: "moonshot", tone: "promotional", suggestion: "Avoid hype language" },
    { pattern: "to the moon", tone: "promotional", suggestion: "Avoid hype language" },
    { pattern: "guaranteed", tone: "promotional", suggestion: "Never guarantee outcomes" },
    // AI-slop markers (corporate filler)
    { pattern: "in the ever-evolving", tone: "formal", suggestion: "AI-slop: delete this phrase entirely" },
    { pattern: "it's important to note", tone: "formal", suggestion: "AI-slop: just state the thing" },
    { pattern: "let's dive in", tone: "formal", suggestion: "AI-slop: just start" },
    { pattern: "dive deep", tone: "formal", suggestion: "AI-slop: use 'explore' or 'examine' or just do it" },
    { pattern: "unpack", tone: "formal", suggestion: "AI-slop: use 'explain' or 'break down'" },
    { pattern: "game-changer", tone: "formal", suggestion: "AI-slop: show don't tell - explain the actual impact" },
    { pattern: "game changer", tone: "formal", suggestion: "AI-slop: show don't tell - explain the actual impact" },
    { pattern: "revolutionary", tone: "formal", suggestion: "AI-slop: be specific about what's new" },
    { pattern: "cutting-edge", tone: "formal", suggestion: "AI-slop: be specific about the technology" },
    { pattern: "cutting edge", tone: "formal", suggestion: "AI-slop: be specific about the technology" },
    { pattern: "leverage", tone: "formal", suggestion: "AI-slop: use 'use' instead" },
    { pattern: "synergy", tone: "formal", suggestion: "AI-slop: describe the actual benefit" },
    { pattern: "holistic", tone: "formal", suggestion: "AI-slop: be specific about what's included" },
    { pattern: "unlock the power", tone: "formal", suggestion: "AI-slop: state the benefit directly" },
    { pattern: "take .* to the next level", tone: "formal", suggestion: "AI-slop: be specific about the improvement" },
    { pattern: "in today's fast-paced", tone: "formal", suggestion: "AI-slop: delete this phrase entirely" },
    { pattern: "without further ado", tone: "formal", suggestion: "AI-slop: just start" },
    { pattern: "first and foremost", tone: "formal", suggestion: "AI-slop: just say 'first' or remove" },
    { pattern: "at the end of the day", tone: "formal", suggestion: "AI-slop: state your conclusion directly" },
    { pattern: "moving forward", tone: "formal", suggestion: "AI-slop: be specific about what's next" },
    { pattern: "robust", tone: "formal", suggestion: "AI-slop: be specific about what makes it strong" },
    { pattern: "seamless", tone: "formal", suggestion: "AI-slop: describe the actual experience" },
    { pattern: "comprehensive", tone: "formal", suggestion: "AI-slop: be specific about what's covered" },
    { pattern: "excited to announce", tone: "formal", suggestion: "AI-slop: just announce it" },
    { pattern: "thrilled to share", tone: "formal", suggestion: "AI-slop: just share it" },
    { pattern: "passionate about", tone: "formal", suggestion: "AI-slop: show passion through substance" },
    { pattern: "ecosystem", tone: "formal", suggestion: "AI-slop: be specific - protocol? community? tooling?" },
    { pattern: "empower", tone: "formal", suggestion: "AI-slop: state what users can actually do" },
    { pattern: "streamline", tone: "formal", suggestion: "AI-slop: be specific about what's faster/easier" },
    { pattern: "optimize", tone: "formal", suggestion: "AI-slop: be specific about the improvement" },
  ],
  
  prohibited: [
    { phrase: "financial advice", reason: "Legal liability", suggestion: "Add 'not financial advice' disclaimer if discussing investments" },
    { phrase: "get rich", reason: "Hype language", suggestion: "Focus on education, not wealth promises" },
    { phrase: "easy money", reason: "Hype language", suggestion: "Nothing in crypto is easy money" },
    { phrase: "can't lose", reason: "Misleading", suggestion: "All investments carry risk" },
    { phrase: "100x", reason: "Hype language", suggestion: "Avoid specific return predictions" },
    { phrase: "shill", reason: "Negative connotation", suggestion: "Use 'recommend' or 'highlight'" },
    { phrase: "ponzi", reason: "Legal/defamation risk", suggestion: "Use 'unsustainable model' if needed" },
    { phrase: "scam", reason: "Legal/defamation risk", suggestion: "Use 'concerning' or 'questionable' unless proven" },
  ],
  ...
  customRules: [
    ...
    },
    {
      ...
      message: "Possible passive voice. Consider active voice for stronger writing.",
      severity: "info",
    },
    {
      name: "Weasel words",
      pattern: "\\b(?:some|many|most|often|usually|generally|probably|possibly)\\b",
...
export function loadStyleGuide(): StyleGuide {
  ...
  try {
    if (fs.existsSync(STYLE_CACHE_PATH)) {
      const cached = JSON.parse(fs.readFileSync(STYLE_CACHE_PATH, "utf-8")) as StyleGuide;
  ...
  if (fs.existsSync(STYLE_GUIDE_PATH)) {
    try {
      ...
      logger.warn("[StyleGuide] Could not parse style guide markdown, using default");
    }
  }
...
function parseStyleGuideMarkdown(content: string): StyleGuide {
  ...
  if (termSection) {
    ...
    const termMatches = termSection[0].matchAll(/[*-]\s*(?:Use\s+)?["']?(\w+)["']?\s*(?:not|instead of|avoid)\s*["']?([^"'\n]+)["']?/gi);
  ...
  const prohibitedSection = content.match(/##\s*(?:Prohibited|Never Use|Avoid)[\s\S]*?(?=##|$)/i);
  if (prohibitedSection) {
    const prohibited: ProhibitedPhrase[] = [];
    const matches = prohibitedSection[0].matchAll(/[*-]\s*["']?([^"'\n:]+)["']?\s*[-:]\s*(.+)/g);
 ...
 */
export function checkStyle(content: string, guide?: StyleGuide): StyleCheckResult {
  ...
  for (const marker of styleGuide.toneMarkers) {
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      violations.push({
        ...
        type: "tone",
        severity: "info",
        message: marker.suggestion || `"${marker.pattern}" has ${marker.tone} tone`,
        line: lineNum,
        found: match[0],
  ...
  const score = Math.max(0, 100 - (summary.errors * 10) - (summary.warnings * 3) - (summary.info * 1));
  
  // Generate suggestions
  const suggestions: string[] = [];
  if (summary.errors > 0) {
    suggestions.push(`Fix ${summary.errors} error(s) — these are critical style violations`);
  }
  if (summary.warnings > 5) {
    suggestions.push(`Address terminology and capitalization warnings for brand consistency`);
  }
  const toneViolations = violations.filter(v => v.type === "tone");
  if (toneViolations.length > 3) {
    suggestions.push(`Review tone markers — ${toneViolations.length} instances may not match brand voice`);
  }
 ...
 */
export function autoFix(content: string, guide?: StyleGuide): { content: string; fixes: number } {
  const styleGuide = guide || loadStyleGuide();
  let fixed = content;
  let fixes = 0;
