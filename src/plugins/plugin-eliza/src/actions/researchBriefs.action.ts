 ...
 */
function searchKnowledge(topic: string): Array<{ file: string; content: string; relevance: number }> {
  ...
  function scanDir(dir: string) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && !entry.name.startsWith(".") && entry.name !== "briefs") {
        scanDir(fullPath);
 ...
 */
function extractExcerpts(content: string, topic: string, maxExcerpts = 3): string[] {
 ...
 */
function generateBrief(topic: string, sources: Array<{ file: string; content: string; relevance: number }>): ResearchBrief {
  ...
  for (const source of topSources) {
    ...
    if (excerpts.length > 0) {
      briefSources.push({
        file: source.file,
        relevance: source.relevance > 10 ? "high" : source.relevance > 3 ? "medium" : "low",
        excerpt: excerpts[0] || "",
      });
    }
  }
  ...
  const highRelevanceSources = briefSources.filter(s => s.relevance === "high").length;
  const confidence: "high" | "medium" | "low" = 
    highRelevanceSources >= 3 ? "high" :
    highRelevanceSources >= 1 || briefSources.length >= 3 ? "medium" : "low";
  
  // Identify gaps
  const gaps: string[] = [];
  if (briefSources.length < 3) {
    gaps.push(`Limited sources (${briefSources.length}) — consider ingesting more content on ${topic}`);
  }
  if (confidence === "low") {
  ...
  return {
    ...
    key_findings: keyFindings.length > 0 ? keyFindings : ["Insufficient data — ingest more content on this topic"],
    implications: ["Analysis requires LLM synthesis — findings extracted from raw sources"],
    recommended_actions: [
      briefSources.length < 5 ? `Expand knowledge base with more ${topic} content` : `Review ${briefSources.length} sources for deeper insights`,
...
Briefs are saved to knowledge/briefs/ for reference.`,
  ...
  handler: async (
    ...
    if (textLower.includes("list brief") || textLower.includes("brief history")) {
      ...
      const briefList = briefs.slice(0, 10).map((b, i) => {
        const age = Date.now() - new Date(b.createdAt).getTime();
        const ageStr = age < 3600000 ? `${Math.floor(age / 60000)} min ago` :
                       age < 86400000 ? `${Math.floor(age / 3600000)} hours ago` :
    ...
    // Extract topic
    const topicMatch = text.match(/(?:brief(?:\s+(?:me\s+)?on)?|quick\s+research[:\s]+|summarize\s+research\s+on)\s+(.+)/i);
    const topic = topicMatch?.[1]?.trim() || text.replace(/research|brief|quick|summarize|on|me/gi, "").trim();
    ...
    // Format response
    const confidenceEmoji = brief.confidence === "high" ? "🟢" : brief.confidence === "medium" ? "🟡" : "🔴";
...
**Top Sources:**
${brief.sources.slice(0, 5).map(s => `• \`${s.file}\` (${s.relevance})`).join("\n")}
