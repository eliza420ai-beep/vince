...
function checkStyleTest(content: string): {
...
describe("Style Guide - Terminology", () => {
  ...
  it("should not flag correct terminology", () => {
    ...
    const termViolations = result.violations.filter(
      (v) => v.type === "terminology",
    );
...
describe("Style Guide - Capitalization", () => {
  ...
  it("should not flag correct capitalization", () => {
    ...
    const capViolations = result.violations.filter(
      (v) => v.type === "capitalization",
    );
...
describe("Style Guide - Prohibited Phrases", () => {
  ...
  it("should detect 'guaranteed'", () => {
    ...
    const violation = result.violations.find(
      (v) => v.found?.toLowerCase() === "guaranteed",
    );
...
describe("Style Guide - Tone Markers", () => {
  it("should detect 'obviously'", () => {
    ...
    const violation = result.violations.find(
      (v) => v.found?.toLowerCase() === "obviously",
    );
...
describe("Style Guide - AI Slop Detection", () => {
  it("should detect 'game-changer'", () => {
    ...
    const violation = result.violations.find((v) =>
      v.found?.toLowerCase().includes("game-changer"),
    );
  ...
  it("should detect 'in the ever-evolving'", () => {
    const result = checkStyleTest(
      "In the ever-evolving landscape of crypto...",
    );
    const violation = result.violations.find((v) =>
      v.found?.toLowerCase().includes("ever-evolving"),
    );
  ...
  it("should detect 'let's dive in'", () => {
    ...
    const violation = result.violations.find((v) =>
      v.found?.toLowerCase().includes("dive in"),
    );
  ...
  it("should detect 'leverage' as AI-slop verb", () => {
    ...
    const violation = result.violations.find(
      (v) => v.found?.toLowerCase() === "leverage",
    );
  ...
  it("should detect 'seamless'", () => {
    ...
    const violation = result.violations.find(
      (v) => v.found?.toLowerCase() === "seamless",
    );
  ...
  it("should detect 'excited to announce'", () => {
    ...
    const violation = result.violations.find((v) =>
      v.found?.toLowerCase().includes("excited to announce"),
    );
...
describe("Style Guide - Auto-Fix", () => {
  ...
  it("should fix multiple issues", () => {
    const { content, fixes } = autoFixTest(
      "cryptocurrency on ethereum with defi",
    );
...
describe("Integration - Fix and Verify", () => {
  it("should fix content and improve score", () => {
    ...
    expect(fixedResult.violations.length).toBeLessThan(
      originalResult.violations.length,
    );
  });
});
...
describe("Real styleGuide.service", () => {
  it("checkStyle detects terminology violation (cryptocurrency -> crypto)", async () => {
    ...
    const result = checkStyle(
      ...
      minimalGuide as Parameters<typeof checkStyle>[1],
    );
    expect(result.violations.length).toBeGreaterThan(0);
    const termViolation = result.violations.find(
      (v) => v.type === "terminology",
    );
