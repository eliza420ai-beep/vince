...
describe("BANKR_AGENT_PROMPT", () => {
  const action = bankrPlugin.actions.find(
    (a) => a.name === "BANKR_AGENT_PROMPT",
  )!;
...
describe("Order type coverage", () => {
  const promptAction = bankrPlugin.actions.find(
    (a) => a.name === "BANKR_AGENT_PROMPT",
  )!;
