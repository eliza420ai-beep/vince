...
function patchMessageServiceForStandupSkip(runtime: any, attempt = 0): void {
  ...
  if (!svc || typeof svc.handleMessage !== "function") {
    if (attempt < PATCH_MESSAGE_SERVICE_MAX_RETRIES) {
      const delayMs =
        PATCH_RETRY_DELAYS_MS[
          Math.min(attempt, PATCH_RETRY_DELAYS_MS.length - 1)
        ];
      if (attempt === 0) {
        logger.warn(
          "[ONE_TEAM] No messageService.handleMessage to patch for " +
            myName +
            " — will retry (messageService registers after services start)",
        );
      }
      setTimeout(() => {
        try {
    ...
    if (roomId) {
      ...
      if (isStandup === undefined) {
        try {
          ...
          isStandup = channelParts.some((p: string) =>
            roomNameLower.includes(p),
          );
        } catch {
          isStandup = false;
        }
  ...
  logger.info(
    `[ONE_TEAM] Patched messageService for ${myName} — standup messages skip processing`,
  );
}
    ...
    if (isStandupCoordinator(runtime)) {
      ...
      setImmediate(() => {
        registerStandupTask(runtime).catch((err) => {
          logger.error(
            { err, agent: runtime.character?.name },
            "[ONE_TEAM] registerStandupTask failed",
          );
        });
      });
    }
    ...
    if (myNameLower !== getFacilitatorName()) {
      ...
      setTimeout(() => {
        ...
        svc.handleMessage = async function kellyStandupWrapper(
          ...
          if (roomId) {
            ...
            if (isStandup) {
              ...
              if (rawText && isStandupKickoffRequest(rawText)) {
                ...
                logger.info(
                  `[KELLY_STANDUP] Standup kickoff: overriding message text so LLM outputs STANDUP_FACILITATE`,
                );
              } else {
                logger.info(
                  `[KELLY_STANDUP] Injected isMention for standup message: "${rawText.slice(0, 50)}"`,
                );
              }
            }
          }
          ...
          const timeoutMs =
            roomId && standupRoomCacheKelly.get(roomId)
              ? getStandupSessionTimeoutMs()
              : 120_000;
          try {
              ...
              new Promise<never>((_, reject) =>
                setTimeout(
                  () =>
                    reject(
                      new Error(
                        `Kelly handleMessage timeout (${timeoutMs / 1000}s)`,
                      ),
                    ),
                  timeoutMs,
                ),
            ...
            logger.info(
              `[KELLY_STANDUP] handleMessage DONE in ${Date.now() - t0}ms`,
            );
            return result;
          } catch (err: any) {
            logger.error(
              `[KELLY_STANDUP] handleMessage FAILED after ${Date.now() - t0}ms: ${err?.message ?? err}`,
            );
            return {
              didRespond: false,
              responseContent: null,
        ...
        logger.info(
          "[ONE_TEAM] Kelly: standup isMention injection + timeout (standup=session, else 120s) active",
        );
      }, 5000);
    }
  },
