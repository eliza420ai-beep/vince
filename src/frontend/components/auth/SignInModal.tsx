...
export function SignInModal({ isOpen }: SignInModalProps) {
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.message || err?.errorMessage || "Failed to send OTP";
  ...
  const handleSmsSubmit = async () => {
    ...
    try {
      ...
      const result = await smsHook.signInWithSms({
      ...
      const errorMessage =
        err?.message || err?.errorMessage || "Failed to send SMS OTP";
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleEmailOtpSubmit = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.message || err?.errorMessage || "Invalid OTP code";
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.message || err?.errorMessage || "Failed to start Google sign-in";
      setError(errorMessage);
      setIsLoading(false);
    }
  };
