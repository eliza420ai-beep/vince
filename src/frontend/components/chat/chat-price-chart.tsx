...
export function ChatPriceChart({ data }: ChatPriceChartProps) {
  ...
  const getEvenlySpacedTimeTicks = (
    dataPoints: PriceDataPoint[] | MarketCapDataPoint[],
    count: number,
  ): number[] => {
  ...
  const hasMarketCapData =
    data.market_cap_data_points && data.market_cap_data_points.length > 0;
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          {activeChartType === "marketcap" &&
            marketCapChange &&
            marketCapChange.percentage !== null &&
            marketCapChange.percentage !== undefined && (
              <div
                className={cn(
                  "flex items-center gap-1 text-sm font-medium",
                  isPositive ? "text-green-500" : "text-red-500",
        ...
        {activeChartType === "price" && data.data_points.length > 0 ? (
          ...
          >
            ...
            >
              <defs>
                <linearGradient
                  id={`fillPrice-${data.token_identifier}`}
                  x1="0"
                  y1="0"
                  x2="0"
                  y2="1"
              ...
              <XAxis
                ...
                tickFormatter={(ts) =>
                  data.data_points.find((d) => d.timestamp === ts)?.date || ""
                }
                interval={0}
                tickLine={false}
                tickMargin={12}
          ...
          >
            ...
            >
              <defs>
                <linearGradient
                  id={`fillMarketCap-${data.token_identifier}`}
                  x1="0"
                  y1="0"
                  x2="0"
                  y2="1"
              ...
              <XAxis
                ...
                scale="time"
                domain={["dataMin", "dataMax"]}
                ticks={getEvenlySpacedTimeTicks(
                  data.market_cap_data_points!,
                  10,
                )}
                tickFormatter={(ts) =>
                  data.market_cap_data_points!.find((d) => d.timestamp === ts)
                    ?.date || ""
                }
                interval={0}
                tickLine={false}
