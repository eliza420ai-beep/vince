...
export const CDPWalletCard = forwardRef<CDPWalletCardRef, CDPWalletCardProps>(
  (
    ...
    const getChainSortOrder = (chain: string): number => {
      const index = SUPPORTED_CHAINS.indexOf(
          ...
          try {
            ...
            if (data && data.tokens) {
              setTokens((prevTokens) => {
                ...
                const otherChainTokens = prevTokens.filter(
          ...
          try {
            ...
            if (data && data.nfts) {
              ...
              setNfts((prevNfts) => {
                ...
                const otherChainNfts = prevNfts.filter(
                  (nft) => nft.chain !== chain,
                );
      ...
      <>
        ...
        <Card className="max-h-[calc(100vh-2rem)] w-full flex flex-col">
          <CardHeader className="flex items-center justify-between pl-3 pr-1 relative z-10">
            ...
            >
              {isLoadingHistory || isLoadingNfts || isLoadingTokens
                ? "Loading..."
                : isRefreshing
                  ? "Refreshing..."
                  : "Refresh"}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div
                                    ...
                                    style={{ maxWidth: "35%", minWidth: 0 }}
                                  >
                                    <span
                                      className={`text-xs sm:text-sm font-mono font-medium overflow-hidden text-ellipsis whitespace-nowrap w-full text-right ${
                                        isReceived
                                          ? "text-green-500"
                                          : "text-red-500"
                                      }`}
                                    ...
                                    <span
                                      className="text-[10px] sm:text-xs text-muted-foreground overflow-hidden text-ellipsis whitespace-nowrap w-full text-right"
                                      title={tx.asset}
                                    >
                                      {tx.asset}
                                    </span>
