...
export default function LeaderboardPage({ agentId, agents }: LeaderboardPageProps) {
  ...
  const [polymarketSort, setPolymarketSort] = useState<"yes" | "volume" | "closes">("yes");
                ...
                (() => {
                  ...
                  const sortMarkets = (list: PM[]): PM[] => {
                    ...
                    if (polymarketSort === "yes") {
                      copy.sort((a, b) => (b.yesPrice ?? 0) - (a.yesPrice ?? 0));
                    } else if (polymarketSort === "volume") {
                      copy.sort((a, b) => Number(b.volume ?? 0) - Number(a.volume ?? 0));
                    } else {
                      copy.sort((a, b) => {
                        const ta = a.endDateIso ? new Date(a.endDateIso).getTime() : 0;
                        const tb = b.endDateIso ? new Date(b.endDateIso).getTime() : 0;
                        return ta - tb;
                      });
                    }
                    return copy;
                  ...
                  const openCryptoEtfMarkets = (cryptoEtf?.markets ?? []).filter(
                    (m) => !m.endDateIso || new Date(m.endDateIso).getTime() > Date.now()
                  );
                  return (
                    <>
                      {openCryptoEtfMarkets.length > 0 && (
                        <DashboardCard title="Crypto ETF">
                          <p className="text-sm text-muted-foreground mb-3">
                            {cryptoEtf?.oneLiner ?? "Crypto ETF flows and related markets — same view as polymarket.com/crypto/etf."}
                          </p>
                          <a
                            href={cryptoEtf?.link ?? "https://polymarket.com/crypto/etf"}
                            target="_blank"
                            rel="noopener noreferrer"
