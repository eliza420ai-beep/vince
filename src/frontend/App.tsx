...
import {
  ...
  useLoadingPanel,
} from "./contexts/LoadingPanelContext";
...
async function authenticateUser(
  ...
  try {
    ...
    if (!cdpUserId) {
      throw new Error(
        "CDP userId not available - user may not be authenticated with CDP",
      );
    }
...
function App() {
   ...
   * This ensures the token cannot be reused after logout
   */
  const signOut = useCallback(async () => {
    try {
      // Revoke the token on the server first
      await elizaClient.auth.logout();
      console.log(" JWT token revoked on server");
    } catch (error) {
      // Log but don't block sign out if server logout fails
  ...
  // Guest userId for when CDP is not configured (persistent per browser)
  const GUEST_USER_ID_KEY = "vince-guest-user-id";
  const getOrCreateGuestUserId = (): string => {
    let id = localStorage.getItem(GUEST_USER_ID_KEY);
    if (!id) {
  ...
  // Initialize authentication when CDP sign-in completes; when CDP is not configured, use guest mode
  useEffect(() => {
    if (!import.meta.env.VITE_CDP_PROJECT_ID) {
      // No CDP: use a persistent guest userId so the chat UI and sidebar render
    ...
    async function initAuth() {
      try {
        ...
        const { email: resolvedEmail, username: resolvedUsername } =
          resolveCdpUserInfo(currentUser as CdpUser | undefined, {
            isSignedIn: true,
          });
        const emailForAuth =
          resolvedEmail || `${currentUser?.userId}@cdp.local`;
        const usernameForAuth =
          resolvedUsername ||
          (emailForAuth ? emailForAuth.split("@")[0] : "User");
  ...
  useEffect(() => {
    ...
    if (!import.meta.env.VITE_CDP_PROJECT_ID) {
      // Guest mode: profile already set in auth effect
      setIsLoadingUserProfile(false);
      return;
    }
    ...
    const syncUserEntity = async () => {
      try {
        ...
        const {
          email: cdpEmail,
          username: cdpUsername,
          phoneNumber,
        } = resolveCdpUserInfo(currentUser as CdpUser | undefined, {
          isSignedIn: true,
        });
        const finalEmail =
          cdpEmail || userEmail || `${currentUser?.userId}@cdp.local`;
        const finalUsername =
          cdpUsername ||
          (cdpEmail ? cdpEmail.split("@")[0] : userName) ||
          "User";
        ...
        try {
          ...
          if (error?.status === 404 || error?.code === "NOT_FOUND") {
            ...
            setUserProfile({
              ...
              bio:
                entity.metadata?.bio || "DeFi Enthusiast • Blockchain Explorer",
        ...
        const shouldRandomizeAvatar =
          !currentAvatar || isKrimsonAvatar(currentAvatar);
        ...
        if (needsUpdate) {
          ...
          if (shouldRandomizeAvatar) {
            console.log(
              ` Randomizing avatar for user (was: ${currentAvatar || "none"}, now: ${avatarUrl})`,
            );
          }

          const updated = await elizaClient.entities.updateEntity(
            userId as UUID,
            {
              metadata: {
                ...entity.metadata,
                ...
                phoneNumber:
                  phoneNumber || entity.metadata?.phoneNumber || undefined,
                walletAddress:
                  walletAddress || entity.metadata?.walletAddress || "",
                displayName:
                  entity.metadata?.displayName || finalUsername || "User",
                bio:
                  entity.metadata?.bio ||
                  "DeFi Enthusiast • Blockchain Explorer",
  ...
  useEffect(() => {
    ...
    async function ensureUserServerAndLoadChannels() {
      ...
      try {
        ...
        const sortedChannels = dmChannels.sort(
          (a: Channel, b: Channel) => (b.createdAt || 0) - (a.createdAt || 0),
        );
        setChannels(sortedChannels);

        console.log(
          ` Loaded ${sortedChannels.length} DM channels (sorted by creation time)`,
        );
        sortedChannels.forEach((ch: Channel, i: number) => {
          const createdDate = ch.createdAt
            ? new Date(ch.createdAt).toLocaleString()
            : "Unknown";
          console.log(
            `  ${i + 1}. ${ch.name} (${ch.id.substring(0, 8)}...) - Created: ${createdDate}`,
          );
        });
        ...
        if (sortedChannels.length === 0 && !hasInitialized.current) {
          ...
          hasInitialized.current = true;
          console.log(
            ` Auto-selected latest channel: ${sortedChannels[0].name} (${sortedChannels[0].id.substring(0, 8)}...)`,
          );
        }
      } catch (error: any) {
        console.warn(" Could not load channels:", error.message);
        // When load fails (e.g. guest/no auth), show new-chat UI so user can still use the app
            ...
            <div className="flex flex-col relative w-full gap-1 min-h-0 h-full">
              ...
              <div className="flex items-center lg:items-baseline gap-2.5 md:gap-4 px-4 md:px-6 py-3 md:pb-4 lg:pt-7 ring-2 ring-pop sticky top-header-mobile lg:top-0 bg-background z-10">
                ...
                >
                  ...
                  <span className="text-sm md:text-base text-muted-foreground uppercase">
                    ABOUT
                  </span>
                </button>
              </div>
    ...
    <CDPReactProvider
      config={{
        ...
        authMethods: [
          ...
          "oauth:twitter",
          "oauth:discord",
        ] as any, // Enable all auth methods including Google OAuth
      }}
    >
      <LoadingPanelProvider>
        <ModalProvider>
