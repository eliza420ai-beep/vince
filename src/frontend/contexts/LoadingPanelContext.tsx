...
const LoadingPanelContext = createContext<LoadingPanelContextType | undefined>(
  undefined,
);
...
export function LoadingPanelProvider({ children }: { children: ReactNode }) {
  ...
  const showLoading = useCallback(
    (title: string, messages: string | string[], id?: string) => {
      setState({
        isVisible: true,
        type: "loading",
        title,
...
export function useLoadingPanel() {
  ...
  if (context === undefined) {
    throw new Error(
      "useLoadingPanel must be used within a LoadingPanelProvider",
    );
  }
  return context;
}
