 ...
 * Uses Bearer token from env: X_BEARER_TOKEN (or project .env when run via bun).
 */
...
function getToken(): string {
  ...
  throw new Error(
    "X_BEARER_TOKEN not set. Add to .env or export X_BEARER_TOKEN.",
  );
}
...
export async function thread(
  ...
  try {
    ...
    const rootData = (raw as any).data;
    if (rootData && !Array.isArray(rootData)) {
      const rootTweets = parseTweets({ ...raw, data: [rootData] });
...
export async function getTweet(tweetId: string): Promise<Tweet | null> {
  ...
  if ((raw as any).data && !Array.isArray((raw as any).data)) {
    const parsed = parseTweets({ ...raw, data: [(raw as any).data] });
...
export function filterEngagement(
  ...
  return tweets.filter((t) => {
    if (opts.minLikes != null && t.metrics.likes < opts.minLikes) return false;
    if (opts.minImpressions != null && t.metrics.impressions < opts.minImpressions)
      return false;
    return true;
  });
}
