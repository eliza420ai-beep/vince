...
# Ensure X_BEARER_TOKEN is set (e.g. from project .env or export)
...
- `--save` — save results to `~/clawd/drafts/x-research-{slug}-{date}.md` (or project drafts)
...
- @username2: "[another perspective]" (Likes, Impressions) [Tweet](url)
...
Use `--save` flag or save manually to a project drafts folder or `~/clawd/drafts/x-research-{topic-slug}-{YYYY-MM-DD}.md`.
...
On heartbeat, can run `watchlist check` to see if key accounts posted anything notable. Flag only if genuinely interesting/actionable — don't report routine tweets.
