#!/usr/bin/env bun
/**
 * x-search — CLI for X/Twitter research (Cursor/Claude skill).
 *
 * Commands:
 *   search [options] <query>  Search recent tweets
 ...
 *   watchlist [add|remove|check]  Watchlist management
 *   cache clear               Clear search cache
 *
 * Requires: X_BEARER_TOKEN in env (or load project .env before running).
 */
...
const DRAFTS_DIR = process.env.HOME
  ? join(process.env.HOME, "clawd", "drafts")
  : join(process.cwd(), "data", "drafts");
...
async function cmdSearch(): Promise<void> {
  ...
  const pages = Math.min(parseInt(getOpt("pages") || "1", 10), 5);
  const limit = parseInt(getOpt("limit") || "15", 10);
  ...
  if (!query) {
    console.error("Usage: bun run x-search.ts search \"<query>\" [options]");
    process.exit(1);
  }
...
async function cmdThread(): Promise<void> {
  const tweetId = args[1];
  if (!tweetId) {
    console.error("Usage: bun run x-search.ts thread <tweet_id>");
    process.exit(1);
  }

  const pages = Math.min(parseInt(getOpt("pages") || "2", 10), 5);
...
async function cmdProfile(): Promise<void> {
  ...
  if (!username) {
    console.error("Usage: bun run x-search.ts profile <username>");
    process.exit(1);
  }

  const count = parseInt(getOpt("count") || "20", 10);
...
async function cmdTweet(): Promise<void> {
  const tweetId = args[1];
  if (!tweetId) {
    console.error("Usage: bun run x-search.ts tweet <tweet_id>");
    process.exit(1);
  }
...
async function cmdWatchlist(): Promise<void> {
  ...
  if (sub === "add") {
    ...
    if (!username) {
      console.error("Usage: bun run x-search.ts watchlist add <username> [note]");
      process.exit(1);
    }
    if (wl.accounts.some((a) => a.username.toLowerCase() === username.toLowerCase())) {
  ...
  if (sub === "remove" || sub === "rm") {
    const username = args[2]?.replace(/^@/, "");
    if (!username) {
      console.error("Usage: bun run x-search.ts watchlist remove <username>");
      process.exit(1);
    }
  ...
  if (sub === "check") {
    ...
    for (const acct of wl.accounts) {
      try {
        ...
        console.error(` Error checking @${acct.username}: ${msg}`);
      }
    }
    return;
  }
...
async function cmdCache(): Promise<void> {
  const sub = args[1];
  if (sub === "clear") {
...
Search options:
  ...
  --save                      Save to ~/clawd/drafts/ or project data/drafts
  --json                      Raw JSON output
  --markdown                  Markdown output

Requires: X_BEARER_TOKEN in environment (or project .env).`);
}
