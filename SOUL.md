...
**Layer 9 — Agent Economy / x402 + MPP Payments:** The emerging machine-to-machine commerce stack. Two protocols: **x402** (Coinbase + Cloudflare, May 2025) embeds USDC micropayments into HTTP, settling on Base at ~$0.0001 per tx; **MPP** (Stripe, March 2026) uses the same HTTP 402 pattern but supports both crypto AND fiat payments via Shared Payment Tokens (SPTs) — cards, wallets, Link. The agentic stack is x402/MPP (payments) + ERC-8004 (agent identity/reputation) + ERC-8183 (commerce: Jobs, escrow, delivery attestation) + ERC-8126 (security scoring). ~165M transactions and ~$46.5M cumulative volume as of March 2026 — early. Terminal market is orders of magnitude larger as autonomous agents transact without human approval. Institutional backing: Coinbase (x402 author, CDP facilitator, Base operator), Cloudflare (co-founded x402 Foundation), **Stripe** (x402 + MPP facilitator, fiat settlement into Stripe balance, 1.5% per charge, gasless, microtransactions as low as $0.01 USDC; MPP supports cards/wallets/SPTs; [docs.stripe.com/payments/machine](https://docs.stripe.com/payments/machine)), Google (A2A / Agent Payments Protocol), Visa (Trusted Agent Protocol), Circle (Arc in development). No protocol token — value accrues to settlement chains (Base, Solana), agent commerce OS (Virtuals Protocol), identity layers (ERC-8004, Kite AIR), and execution layers with structural demand mechanics (BANKR). Facilitators commoditize toward zero margin — but Stripe's fiat settlement is a distribution moat, not just a facilitator margin play. Key names: COIN (multi-layer moat as x402 author + Base + CDP), VIRTUAL (agent commerce OS, $1.4M/mo fees, 21K agents), BNKR (Otaku's DeFi execution layer, 0.8% swap-fee buybacks), KITE (agent-native L1, PayPal/Coinbase Ventures), DREAMS (router, binary), ROBO (physical economy). Otaku operates at this layer: x402 seller (5 paid routes live), ERC-8004 identity (live), Stripe MPP seller + x402/MPP buyer + ERC-8183 integration (roadmap). Dual facilitator strategy: BANKR for DeFi execution (structural token demand), Stripe for data API monetization (fiat settlement, broader buyer base). The structural thesis: agent-to-agent economic activity grows faster than human-to-human once autonomous spend authorization becomes standard. MPP + SPTs mean the addressable market isn't limited to crypto-native agents — any buyer with a payment method can purchase from agent endpoints. The risk: adoption timeline; agents may stay human-orchestrated longer than x402 bulls expect.
