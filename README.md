...
### Production stitching (what’s already wired)

VINCE + Dexter + AIHF are connected by explicit artifact contracts and a pre-trade conviction gate:

- **Dexter → VINCE (UI + monitoring):** VINCE reads `portfolio_hyperliquid.json`, `portfolio_tastytrade.json`, and `portfolio_watchlist.json` from `DEXTER_ARTIFACT_ROOT` and shows them in the dashboard (Charts/Watchlist).
- **VINCE → Dexter (regime handoff):** VINCE emits `regime_bundle_v1.json` for Dexter operators (and we have `scripts/verify-regime-bundle-v1.ts` to validate it locally).
- **AIHF Phase D (equity-only conviction gate):** VINCE reads `values.aihfSecondOpinion` and applies an equity-only dampener/booster in `VincePaperTradingService.evaluateAndTrade()`:
  - contradiction: cap `tradeSignal.confidence` to `50`
  - agreement: boost `tradeSignal.strength` slightly
  - always: append an `AIHF` factor into the trade reasons/signals

For Roy’s end-to-end production checklist, sign-off steps, and remaining review items, see `docs/PROD_STACK_SINGLE_SOURCE_OF_TRUTH_ROY.md`.
...
- **Gem scoring:** Cohort-relative momentum, sector-relative when available, plus subscores (sleeve-fit, momentum/trend, catalyst/event, valuation/quality, diversification, regime). **Structured explanations** per ticker: Momentum, Quality, Event, Liquidity, Portfolio fit — so the operator sees _why_ a name made the list in a fixed, comparable format.
