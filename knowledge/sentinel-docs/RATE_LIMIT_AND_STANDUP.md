...
When many agents share one LLM org (e.g. Anthropic 200K input tokens/min), a single human message in a shared channel can trigger every agent to call the model and quickly hit the limit. This doc describes the configuration knobs to stay under rate limits and keep standup useful.
