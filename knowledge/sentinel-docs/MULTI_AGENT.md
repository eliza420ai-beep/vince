...
- **Knowledge → Standup & Day Report:** Recent uploads (last 48h in `knowledge/`, e.g. from Discord #knowledge / #upload via Eliza UPLOAD and summarize) drive Eliza's section in shared daily insights: **Substack idea**, **Knowledge to expand**, and **Research to do**. The Day Report format includes "Essay idea" and "Research" lines, and when shared insights exist they are injected so the report always surfaces these suggestions. That way uploads are visibly leveraged each day.
...
- **North-star deliverables:** Action items can also be parsed as **essay** (Substack), **tweets** (banger suggestions), **x_article** (long-form for X), **trades** (perps Hyperliquid + options HypeSurface for BTC/SOL/ETH/HYPE), **good_life** (Kelly-style founder lifestyle suggestions), **prd** (Sentinel: PRD for Cursor), **integration_instructions** (Sentinel: Milaidy/OpenClaw setup and integration), or **research** (Eliza: research brief for RESEARCH_BRIEF/AUTO_RESEARCH). These are generated in-VINCE and written to subdirs under `STANDUP_DELIVERABLES_DIR`. See [docs/NORTH_STAR_DELIVERABLES.md](docs/NORTH_STAR_DELIVERABLES.md).
