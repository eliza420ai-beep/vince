 ...
 * - workspace/HEARTBEAT.md, workspace/BOOT.md, workspace/AGENTS.md
 *
 * If Brain/Muscles/Bones/DNA/Soul output exists (workspace/USER.md or knowledge/teammate/USER.md),
...
const PROMPT_PATH = path.join(EYES_DIR, "EYES_PROMPT.md");
...
function getSystemPrompt(): string {
  if (!existsSync(PROMPT_PATH)) {
    throw new Error(`Eyes prompt not found: ${PROMPT_PATH}`);
  }
...
function getPriorContextSummary(): string | null {
  const candidates = [
    ...
    path.resolve(EYES_DIR, "..", "..", "knowledge", "teammate", "USER.md"),
  ];
...
/**
 * Extract HEARTBEAT.md, BOOT.md, and AGENTS.md from assistant text.
 */
function extractWorkspaceFiles(assistantText: string): Map<string, string> {
  const out = new Map<string, string>();
  const names = EYES_WORKSPACE_FILES.join("|");
...
function writeWorkspaceFiles(extracted: Map<string, string>): void {
  ...
  for (const name of EYES_WORKSPACE_FILES) {
  ...
  for (;;) {
    ...
    if (line === "/done" || line === "/generate") {
      ...
      if (extracted.size > 0) {
        ...
        console.log("\nNo HEARTBEAT.md, BOOT.md, or AGENTS.md blocks found in the response. You can copy-paste into openclaw-agents/workspace/ manually.");
      }
      process.exit(0);
    }
    if (!line) continue;
