...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **WTT daily task**  | After the markdown report, the task runs LLM extraction to produce a typed **WttPick** (thesis, primaryTicker, primaryDirection, primaryInstrument, rubric, invalidateCondition, killConditions, alt\*, evThresholdPct). Saved as `docs/standup/whats-the-trade/YYYY-MM-DD-whats-the-trade.json` alongside the `.md`. When extraction and narrative fallback both fail, a markdown fallback parses the saved report (Structured Pick line or narrative/thesis) and writes JSON when possible. **If JSON is still missing** after the task, re-run the ECHO "what's the trade" action (or re-trigger the daily task) to re-run extraction; the paper bot skips WTT evaluation until the JSON exists. |
