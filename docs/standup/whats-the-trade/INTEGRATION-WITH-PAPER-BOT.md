...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **WTT daily task**  | After the markdown report, the task runs LLM extraction to produce a typed **WttPick** (thesis, primaryTicker, primaryDirection, primaryInstrument, rubric, invalidateCondition, killConditions, alt\*, evThresholdPct). Saved as `docs/standup/whats-the-trade/YYYY-MM-DD-whats-the-trade.json` alongside the `.md`. When extraction and narrative fallback both fail, a markdown fallback parses the saved report (Structured Pick line or narrative/thesis) and writes JSON when possible. **If even that fails, the task now always writes a no-pick placeholder JSON** (`primaryTicker: "NONE"`) so the file exists; the paper bot skips WTT for that day (ticker not in universe). Re-running the ECHO "what's the trade" action is only needed if the task itself did not run. |
