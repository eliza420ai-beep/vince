...
- **SOLUS_ANALYZE (upgraded):** Stock analysis now emits structured score bands (`ThesisStrength`, `CatalystDensity`, `ValuationStretch`, `ExecutionRisk`, `NetEdgeScore`) and deterministic recommendation (`accumulate`, `watch`, `avoid`) with invalidation.
- **SOLUS_THEME_RADAR:** Weekly “outside-the-box” scan for AI bottleneck trades: top 3 opportunities + top 2 crowded traps with causal chains, catalyst windows, confidence, invalidation.
...
- **Stock memory + calibration:** Solus records stock calls to `.elizadb/solus/solus-stock-recommendations.jsonl` and runs daily stock calibration (`SOLUS_STOCK_CALIBRATION`) to auto-resolve mature calls, compute score-bucket hit-rate/returns, and inject notes via `SOLUS_STOCK_CALIBRATION_CONTEXT`.
...
- **Outside-the-box stock framework:** Solus now prioritizes **AI bottlenecks** (power access, permits, interconnect queues, hosting conversion, capex timing) and second-order beneficiaries. Every stock thesis should state a causal chain (event → bottleneck → business impact) plus an explicit invalidation.
