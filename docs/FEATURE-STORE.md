...
| **Monitor progress**                        | Feature store exposes `getCompleteRecordCount()` and `getAvoidedRecordCount()`. The training task logs “Feature store: N closed, M avoided (need 90+ closed for training)”. Check logs or add a periodic log: “Feature store: N closed, M avoided (last 24h)” to see how fast you’re collecting. | —                                                                                                                                                   |

**Data sufficiency:** "Enough" data for the paper bot means 90+ closed trades so ONNX training can run; for better generalization, aim for multiple assets, regimes, and (optionally) WTT coverage. Avoided records add context but training uses only closed trades. See "When training runs" below.
...
**Optional columns:** All optional feature columns used by `train_models.py` (`OPTIONAL_FEATURE_COLUMNS` and news `news_nasdaqChange`, `news_etfFlowBtc`, `news_etfFlowEth`) are populated when data exists: `collectMarketFeatures()` (dvol, fundingDelta, bookImbalance, bidAskSpread, priceVsSma20, rsi14, oiChange24h), `collectNewsFeatures()` (etfFlowBtc/EtfFlowEth from headlines -1/0/1; nasdaqChange from HIP-3/Yahoo; macroRiskEnvironment from HIP-3), `collectSignalFeatures()` (xSentimentScore, hasOICap), WTT block, and postMortem fields at outcome.
...
Feature-store JSONL is the primary input for **VinceBench**, a reproducible benchmark that scores _decision quality_ (process, not PnL) via typed signatures across domains (signal quality, risk discipline, timing, regime, etc.). Run from repo root or plugin directory:
