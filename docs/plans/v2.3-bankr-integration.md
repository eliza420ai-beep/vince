# v2.3 Plan — BANKR Deep Integration

**Status:** In Progress (tests ✅, Otaku integration ✅)
**Created:** 2026-02-12
**Branch:** `feature/v2.3-bankr-integration`

---

## Current State

### plugin-bankr (Agent API)
- **10 actions:** prompt, user info, job status/cancel, sign, submit, order quote/list/status/cancel
- **2 services:** BankrAgentService, BankrOrdersService
- **Capabilities:** Natural language → trading jobs, limit/stop/DCA/TWAP orders, signing, tx submission
- ✅ **92 tests** (5 test files)

### plugin-bankr-sdk
- **1 action:** prompt with own wallet
- **1 service:** BankrSdkService
- ⏳ Tests pending

### plugin-otaku (NEW)
- **4 actions:** OTAKU_SWAP, OTAKU_LIMIT_ORDER, OTAKU_DCA, OTAKU_POSITIONS
- **1 service:** OtakuService (wraps BANKR with confirmation flows)
- ✅ **45 tests** (2 test files)

---

## Why BANKR Matters for VINCE

From `knowledge/sentinel-docs/BANKR.md`:

> "Token pays the agent: compute, execution, subscriptions. Wallet + swap/limit/perps in the product."

BANKR provides:
1. **Execution layer** — swap, bridge, limit, stop, DCA, TWAP, perps
2. **Wallet management** — cross-chain (Base, Solana, Polygon, Ethereum)
3. **Natural language trading** — "swap 1 ETH for USDC" just works
4. **Token economics** — $BNKR funds the agent

---

## v2.3 Proposals

### 1. 🧪 Tests for BANKR Plugins (Priority: HIGH)

Both plugins have zero tests. Need:

```
plugin-bankr/src/__tests__/
├── bankr-agent.service.test.ts    # Mock API, test prompt/job/sign
├── bankr-orders.service.test.ts   # Mock API, test quote/list/cancel
├── actions.test.ts                # Action validation
└── types.test.ts                  # Type validation
```

**Estimated:** 4-6 test files, ~800 lines

---

### 2. 🐙 Otaku + BANKR Deep Integration (Priority: HIGH)

Otaku is "the only agent with a funded wallet" — should use BANKR for everything.

**Current:** Otaku has DeFi capabilities but execution is fragmented
**Proposed:** BANKR becomes Otaku's execution layer

```typescript
// Example: Otaku uses BANKR for all swaps
"Otaku, swap 0.5 ETH for USDC on Base"
→ BANKR_AGENT_PROMPT action
→ Job polling
→ Transaction signed and submitted
→ Confirmation to user
```

**New Otaku actions:**
| Action | Description |
|--------|-------------|
| `OTAKU_SWAP` | Swap via BANKR (wraps BANKR_AGENT_PROMPT) |
| `OTAKU_LIMIT_ORDER` | Set limit order via BANKR |
| `OTAKU_DCA` | Set up DCA via BANKR |
| `OTAKU_POSITIONS` | Show BANKR positions |

---

### 3. 📊 Dashboard Integration (Priority: MEDIUM)

Add BANKR to the leaderboard/dashboard:

**New routes:**
- `/vince/bankr/positions` — Active positions
- `/vince/bankr/orders` — Open orders
- `/vince/bankr/jobs` — Recent job history

**Leaderboard tab:** "Bankr" section showing:
- Open orders (limit, stop, DCA, TWAP)
- Recent trades
- Position summary

---

### 4. 📡 Provider for VINCE (Priority: MEDIUM)

VINCE should know about BANKR orders as part of "objective data."

**New provider:** `bankrOrdersProvider`
- Fetches open orders on request
- Provides to VINCE ALOHA as context
- "You have 2 open limit orders on BTC"

---

### 5. ⚖️ Solus + BANKR (Priority: LOW)

Solus does execution planning (size/skip/watch). BANKR could be the execution layer.

**Flow:**
```
User → "I want to long BTC"
→ VINCE gives data context
→ Solus gives size/skip/watch + invalidation
→ Otaku executes via BANKR with stop loss
```

This requires cross-agent coordination (ASK_AGENT) working well.

---

### 6. 🔌 OpenClaw Skill (Priority: LOW)

Publish VINCE as a skill in [openclaw-skills](https://github.com/BankrBot/openclaw-skills).

Others can install VINCE's capabilities:
- ALOHA action
- Paper bot
- Options analysis

This is more about ecosystem contribution than core functionality.

---

## Recommended v2.3 Scope

**Done:**
1. ✅ Tests for plugin-bankr (5 test files, 92 tests)
2. ✅ Otaku + BANKR integration (plugin-otaku: 4 actions, 45 tests)
3. ✅ VINCE provider for orders (bankrOrdersProvider)
4. ✅ Tests for plugin-bankr-sdk (2 test files, 27 tests)

**Nice to have:**
5. ⏳ Dashboard BANKR tab

**Future (v2.4+):**
6. Solus + BANKR execution flow
7. OpenClaw skill publication

---

## Completed Work

### plugin-bankr Tests (Already existed!)
| File | Tests |
|------|-------|
| `types.test.ts` | 19 |
| `bankr-agent.service.test.ts` | 21 |
| `bankr-orders.service.test.ts` | 15 |
| `bankr.provider.test.ts` | 8 |
| `actions.test.ts` | 29 |
| **Total** | **92** |

### plugin-otaku (NEW)
| File | Tests |
|------|-------|
| `otaku.service.test.ts` | 13 |
| `actions.test.ts` | 32 |
| **Total** | **45** |

### plugin-bankr-sdk (NEW)
| File | Tests |
|------|-------|
| `bankr-sdk.service.test.ts` | 11 |
| `actions.test.ts` | 16 |
| **Total** | **27** |

### plugin-vince
| File | Description |
|------|-------------|
| `bankrOrders.provider.ts` | Cross-agent: BANKR orders in VINCE context |

**Combined: 164 tests for BANKR ecosystem**

---

## Estimated Remaining Effort

| Item | Files | Lines | Time |
|------|-------|-------|------|
| Dashboard BANKR tab | 4 | ~600 | 2h |

**Remaining:** ~4 files, ~600 lines (optional)

---

## Decisions Made

1. **Otaku = BANKR executor** — No need for 9th agent. Otaku already owns "funded wallet" lane.
2. **All features** — DCA, TWAP, Perps all supported via BANKR_AGENT_PROMPT
3. **Tests + Integration in parallel** — Both done together
4. **plugin-otaku approach** — High-level wrapper actions with confirmation flows, not just pass-through
