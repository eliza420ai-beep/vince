...
| `LIVE_EXECUTION_MODE`                                      | Global live execution status for Otaku: `off` (default, no live orders), `shadow` (simulated only), `pilot` (capped live bucket), or `prod` (graduated/live, requires PRD sign-off). |
| `OTAKU_LIVE_ENABLED`                                       | Explicit live toggle for Otaku. `false` by default; must be `true` **and** `LIVE_EXECUTION_MODE` set above `off` before any live-capital path is allowed.                          |
