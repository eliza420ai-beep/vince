...
- **PRDs (standup):** [Paper trading algo + ML](standup/prds/PRD_PAPER_TRADING_ALGO_AND_ML.md) — decision flow, gates, how ML improves the algo. [ML training pipeline](standup/prds/PRD_ML_TRAINING_PIPELINE.md) — feature store → train → ONNX → report → Sentinel. [Three-layer stack + Dexter/AIHF + production](standup/prds/PRD_THREE_LAYER_STACK_PRODUCTION_AND_REPO_INTEGRATION.md) — VINCE/Dexter/ai-hedge-fund contracts, sync, Railway gating, phased plan. [X bookmarks + paste.trade + paper overlay](standup/prds/PRD_X_BOOKMARKS_PASTE_TRADE_PAPER_OVERLAY.md) — lower setup friction, schedule, paste.trade bridge, observability, feature-store attribution.
